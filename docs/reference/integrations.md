...
| [Codex CLI](https://github.com/openai/codex)                                         | `codex`          | Skills + prompt + plugin integration; installs executable skills into `.agents/skills/sp-<command>/SKILL.md`, prompt companions into `.codex/prompts/sp.<command>.md`, and a project-local plugin marketplace under `.agents/plugins/`; these files target `/prompt::sp.<command>`, but slash-menu visibility depends on the current Codex client |
