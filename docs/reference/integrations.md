...
| [Codex CLI](https://github.com/openai/codex)                                         | `codex`          | Skills-first integration; use `$` or `/skills` in Codex and choose `sp-<command>`. Installs executable skills into `.agents/skills/sp-<command>/SKILL.md`; prompt companions and the project-local plugin surface (`.agents/plugins/marketplace.json` plus `plugins/sp/`) are compatibility artifacts, not the install success criterion |
