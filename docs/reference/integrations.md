...
| [Codex CLI](https://github.com/openai/codex)                                         | `codex`          | Skills-first integration; use `$sp-<command>` or `/skills` in Codex and choose `sp-<command>`. Natural-language requests may also invoke a matching skill when the task matches the skill description. Installs executable skills into `.agents/skills/sp-<command>/SKILL.md`; deprecated prompt/plugin command surfaces are not part of the success criterion |
