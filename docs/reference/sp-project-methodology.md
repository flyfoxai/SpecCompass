...
renderer directory / renderer 目录仍是多文件固定基础设施。固定 renderer 的短参数入口是 `.specify/review/renderer/speccompass-review-renderer.html?flow=<feature>` 和 `.specify/review/renderer/speccompass-review-renderer.html?ui=<feature>`；批次 Markdown 只是失败时的文本兜底。交互复核必须在项目根目录启动固定服务：flow 使用 `node .specify/review/scripts/serve-review.mjs --flow <feature>`，UI 使用 `node .specify/review/scripts/serve-review.mjs --ui <feature>`。启动器默认绑定 `127.0.0.1`；需要同一内网的其他设备访问时，可显式追加 `--host <RFC1918-private-ip>`（例如 `--host 10.0.0.209`）。公网地址、域名和 `0.0.0.0` 会被拒绝。只有在 renderer 和 review data 均返回 HTTP 200 后才输出 `SPECCOMPASS_REVIEW_URL=`；模型必须保持该进程运行并把原样 URL 交给用户，不能猜测或替换端口。内网模式会打印警告，审核完成后应停止服务，因为同一内网中可达设备可能读取当前复核数据并尝试提交本次写回；静态访问白名单只包含固定 renderer、需求导航使用的同类型结构化 review data 和 `specs/review-index.json`，不提供其他项目文件。交互复核禁止使用 `file://`，并且 `localhost` 不接受；HTTPS、`::1` 和其他主机名同样不接受。已有项目如缺少这套固定基础设施，使用 `specify init --force` 刷新 `.specify/review/`；`specs/` 下的项目 review data 和确认文档不属于该固定目录。
