...
需求入口也遵循这个原则。`/sp.prd` 是所有新 feature、能力方向和重要需求变更的强制上游入口，用于从 0 到 1 的需求发散、访谈式收集、候选需求隔离和 PRD 草稿沉淀。简单需求可以走精简 PRD，但不能跳过 PRD。精简 PRD 只适用于用户已经给出清楚目标、用户、范围和基本验收意图的情况，最少也要保留战略目标、目标用户、核心场景、范围/非目标、验收种子、风险/open questions 和 handoff；0 到 1 想法、范围不清、多能力方向、治理影响、高风险或 source 冲突必须走完整 PRD。`prd.md` 是需求来源容器；仓库根 `prd/` 是默认业务资料根；`specs/<feature>/prd.md` 是当前 feature 对资料和决定的整理落点，不是默认唯一来源。人工可以追加或明确替换资料目录，命令应记录实际来源根和锚点。用户确认、正式来源或已消费人工决定支持的内容是 Outline 不得丢失的需求事实；`[src:ai-proposed]`、`[uncertain:*]` 等明确候选不是事实。`/sp.specify` 仍是稳定规格入口，负责把已确认、可追踪、适合稳定化的内容写入 `spec.md`。如果执行 `/sp.specify` 时缺少当前 feature PRD、资料来源或 PRD readiness，应先返回 `/sp.prd`。

三个设计阶段按输入递增，不按旧产物自我循环：Outline 读取 PRD 资料；Flow 读取 PRD 资料和已确认 Outline；UI 读取 PRD 资料、已确认 Outline 和已确认 Flow。`spec.md`、澄清记录和设计权威在各自阶段提供稳定编译与约束，但不能取代这些上游业务输入。普通重生成不得把旧 Outline、旧 Flow 或旧 UI 当成隐藏事实来源。每层都先读取当前资料，再在已确认上游边界内使用模型能力补齐合理结构。

资料不足时应区分“模型能设计”和“必须由人决定”。不会改变范围、权限、安全、资金、合规、不可逆行为、关键验收或跨项目责任的低影响可逆选择，由模型记录来源与理由后继续。会造成重大业务影响或大范围返工的决定，必须放到所属 Outline、Flow 或 UI 的 SP Web 审核页，提供 2-4 个真实方案、影响和模型推荐；推荐不等于授权。Outline、Flow 和 UI 三个设计阶段都确认后，Plan、Tasks 和 Implement 应更多依照它们形成的框架自行完成技术分解、任务和代码，只有发现上游冲突、新的重大决定或确认已经过期时才回退。
...
`/sp.prd` 收尾时必须做 PRD-to-spec outline readiness 检查：如果战略目标、用户/角色、范围、能力地图和 source authority 已经清楚，应自动生成或刷新 `specs/<feature>/spec-outline.md`，但先进入 `AWAITING_OUTLINE_CONFIRMATION`，不能直接写成 `READY_FOR_SPECIFY`。如果 PRD 仍存在关键 `[src:ai-proposed]`、`[uncertain:*]`、范围冲突、source 缺失或 feature 边界不清，不能生成稳定 outline；应在 `prd.md` 末尾输出 `Outline Decision`，说明为什么不能稳定化、下一步应走 `/sp.clarify`、继续 `/sp.prd`、补 source，还是拆分 feature。如果 feature 目录已经明确，还应同步创建或刷新阻断型 `specs/<feature>/spec-outline.md`，并在其中保留同一个 `Outline Decision`，让 `/sp.specify`、检查脚本和后续 agent 能从单一入口读到当前阻断原因。

用户第一次进入 `/sp.prd` 时，材料可能还不足以直接形成稳定框架。`outline_maturity` 只表示当前 Outline 是否仍在递归拆分、是否已进入终端单元的详细框定，以及是否已经可以编译正式确认；它与项目树深度、review level、confirmation priority 和 Outline Decision 正交。`explore` 表示当前展开窗口仍在做项目单元拆分，适用于 `000` 和任意更深的父单元；`frame` 表示一个已确认的终端单元正在补齐详细业务 Outline；`specify_ready` 表示来源保真的正式编译已经具备准入条件。不存在固定的“一级/二级/三级项目”语义。

`000` 是真实的顶级 Outline 单元，有自己的目标、范围、整体结果和来源；第一次窗口只生成一个直接后代层。后续从任意普通单元继续拆分时，由模型基于业务复杂度决定生成两层或三层，所有分支提前到达末端时可以少于两层。当前导图最多显示三层只是窗口展示约束，不能把它当成整棵树的最大深度。每个父单元保留完整语义；子单元必须不重叠且完整覆盖父单元的能力和业务链。单元拥有子单元时仍然要写自己的目标和结果；`decomposition_basis` 说明拆分如何降低复杂度，`terminal_basis` 说明继续拆分为何会增加管理、交接或实现复杂度。

当前单元的 Outline 生成先读取父级 handoff 和继承约束，再重新扫描默认 `prd/`、人工指定资料目录、当前 feature PRD、父级引用和本地已确认资料。`.specify/memory/*`、feature `memory/*` 和 `feature-map.md` 只用于路由、历史风险和最小读取集；其中未确认的边界数量、候选名称、owner/lifecycle 或 readiness 摘要不能作为业务证据，也不能直接复用为新分组。handoff 是方向和责任合同，不是完整需求白名单。父子核对检查方向、范围、责任、结果、交接、来源和全局约束的闭合关系，不比较两边是否写出相同节点或数量。只有正式确认的终端单元才进入详细功能框定；资料不足但可以设计的结构标记 `ai-proposed`，缺失的业务事实保留 evidence gap 或进入所属 Web 决策页。
...
每个 `explore` 展开窗口采用三阶段生成。第一阶段从当前展开根的父级方向和继承约束出发，读取用户输入、默认 `prd/`、人工指定资料目录、当前 feature PRD、已接受意图账本和正式业务来源，提取本窗口的 `business_context` 与内部能力原子：真实业务主语 `product_subject`、业务对象 `business_objects`、业务动作 `operations`、可观察结果 `outcomes`、端到端 `business_chains` 和证据缺口。能力原子是“当前单元对哪个业务对象执行什么动作或控制，并产生什么可观察结果”的最小有来源陈述；每个原子明确拥有一个触发或输入、一个业务状态、一个可独立验收结果和一个下游交接，所属业务链必须逐字段保持一致。首次通过校验后，把纯 Stage A 内容保存到 `specs/<feature>/prd/source-capability-baseline.json`；来源指纹、当前人工修正和已接受决定都未变化时，后续重跑必须原样复用这张原子表，只重新执行项目分区。该基线不含项目名称、数量、分组或推荐，不能让旧分区自证。来源发生变化时才重新提取并替换基线。能力覆盖表、阶段名、检查清单和模型自检只用于内部编译，不进入导图、问题、推荐或面向用户的说明。

第二阶段先平铺当前根单元的全部原子和一对一业务链，再按五步完成直接子项目分区。第一步必须运行 `derive-outline-reference-partition.mjs`，按 Stage A 中完全相同的 `responsibility_owner_ref + lifecycle_ref` 把原子投影为责任单元参考分区；脚本同时标出每个 cell 的 `hard`/`soft` 身份权威、整体 `provisional_authority` 和镜像 token 诊断。同一份 Stage A 永远得到同一份参考分区；有文档或人工锚定时，冲突身份是硬不可合并边界；全是 `ai-proposed` 时，参考分区只是确定但低权威的临时默认，不能把模型推断的 owner/lifecycle 当成来源事实。此时必须额外起草一套业务系统边界分区：以软件架构专家身份审核来源文档中命名的模块（如股票数据管理、QMT 网关、OMS、风控引擎、策略管理、研究回测），结合 27 个原子和软件工程最佳实践，判断每个系统是否符合独立状态存储、独立部署生命周期、命名接口三个标准，并根据实际情况调整范围、拆分、合并、增删系统。这套分区是基于来源证据和工程判断的专家建议，不是机械复制源文档标题，也不是主观偏好。随后把参考分区和 1-3 套真正不同、完整且不重叠的备选一起比较；先过权威门禁，再比较内部复杂度与协调成本之和；淘汰技术层、权威跨界、通用容器、内部分类词、填充、数量目标和运行时拓扑边界；最后才输出选中子项目及证据。当 `provisional_authority` 包含 `hard` 或 `user-confirmed` 时，参考分区是选中树（稳定性来自确认的来源身份）；当全是 `ai-proposed` 时，业务系统分区是选中树（架构判断取代机械 token 分组），参考分区仅作为对比项出现在 Discovery 中展示"架构整理前的机械投影"。这样稳定性来自同一 Stage A 的确定性投影（有确认身份时）或软件架构专家判断（无确认身份时），不来自固定项目数量，也不靠重复使用上一轮项目清单。

**业务系统粒度门禁（在全 `ai-proposed` 且参考分区有 7 个或以上子项目、或任意兄弟粒度悬殊时强制触发）：** 第二步备选起草阶段，除了与参考分区实质不同的细粒度备选，还必须额外起草一套**业务系统边界分区**：每个分组对应来源文档中命名的一个可独立运行的软件系统，其判断标准是拥有独立的持久化状态存储（有自己的迁移和保留策略）、可独立部署和替换的技术生命周期、以及通过命名接口（API/命令/事件）与其他系统通信。以来源文档的模块设想为起点，以软件架构专家身份审核：当来源命名的模块实际覆盖多个独立系统或只是一个系统的一部分时调整范围；当模块混合了不同状态所有权或验收标准的职责时拆分；当模块共享相同状态存储且无法独立部署时合并；当原子揭示了来源未命名的系统时增加；当来源命名的模块实际是内部层或横切关注点而非对等系统时删除。这套分区必须完整覆盖全部原子、以完整条目写入 `partition_analysis`，并通过 Step 3 的硬拒绝规则。参考分区有 8 个以上子项目、每个子项目只含 1-3 个原子时，是参考分区处于错误抽象层级的强烈信号，业务系统备选就是将独立可部署系统的实际构建和维护方式显式化的手段。这是基于来源证据和软件工程最佳实践的专家建议，不是机械推导也不是主观偏好。

固定关系仍是：”一个来源能力形成一个能力原子；一个能力原子只归属一个直接子单元；能力原子负责来源覆盖，不负责决定项目数量”。协调成本除显式 handoff、重复状态、验收割裂和重复上下文外，还必须计入每增加一个项目带来的独立 PRD/Outline/Flow/Tasks/Code 上下文、验收边界、交付生命周期和 owner 交接固定成本；没有直接 atom-to-atom handoff 本身不是拆分证据，共享受治理政策、受众、整体验收或变化生命周期的并列原子仍可能形成一个项目。实际分解且确有选择时比较 2-4 套方案，每套分别写明 `internal_complexity`、`coordination_cost` 和 `total_complexity_rank`，并记录 `reference_partition_id` 与 `selected_partition_id`。单原子或没有第二套有效分区时不制造假方案。目标仍是功能单一、松散耦合，但不是默认一原子一项目，也不是默认合成少数大项目。参考分区是选中树的可复现默认，但当业务系统分区比参考分区少 3 个以上子项目时，必须在根 Discovery 问题中将业务系统候选标记为推荐方案，说明理由为”按来源文档命名模块归并后从 X 个项目降至 Y 个，降低协调成本而保留全部业务职责；参考分区仅反映模型推断的 owner/lifecycle token，非来源架构建议”。

每个 Outline 单元必须有自己的业务目标、可核验整体结果、内聚的业务对象/规则/状态和来源，并填写 `project_title` 和 `capability_summary`。`project_title` 是该单元**拥有并治理**的有界业务实体的名字：对于多原子系统级项目（3 个以上原子，符合系统边界标准），提炼为统一的业务职责短语，命名领域和生命周期角色，优先使用”治理/管控/保障/管理”等精确职责词，不要使用通用词”记录”作为区分性词；对于单原子或小集群项目，复用至少一个自有业务对象的具体名词。`capability_summary` 是要点列表（最少 3 条），每条用主动语态动词短语说明系统提供的一项具体能力，包含业务对象和可观察结果，不用段落文本。示例 project_title：错”外部信息与市场背景记录系统”（记录是通用词，且合并了两类职责）→ 对”外部信息治理系统”；错”策略研究资产管理系统”（资产指金融资产）→ 对”策略研究治理系统”；对”行情数据管理系统”；对”OMS与交易风控系统”。`project_title` 不得是 owner/lifecycle slug、”X责任””X生命周期””责任单元/责任域/能力域”等内部分类词；不得以动作或治理词结尾（发布/授权/决定/裁决/披露/评测/审计/恢复/执行/采集/处理/管理/计算/路由/监控），除非该词本身就是来源命名的自有对象；不得用伞词（事实/证据/信息/数据/版本/特征/结果）替换对象的具体名词。父单元不能因为已经拆出孩子而变成空白目录。单元引用的业务链集合必须等于其能力原子引用业务链的集合。生成的多原子子单元只在全部原子真实共享时填写 `shared_responsibility_owner_ref` 和 `shared_lifecycle_ref`；跨越的只是模型推断身份时允许为空，不能为了归组改写 Stage A 所有权，相互冲突的文档或人工确认身份仍不得跨越。除此之外，`grouping_basis` 只保留一句覆盖全部原子的自然语言业务不变量和一个 separation test。反证必须把全部原子完整放入至少两个最佳备选组，记录真实跨组业务事实和确有的重复状态，并用 `decision_reason` 说明为什么当前归组总复杂度更低；不再要求不变量类型枚举、`parent_cohesion`、`keep_together_complexity` 或 `split_coordination_cost` 等平行证明。模型补出的关系保持 `ai-proposed`，声明 `proposal_origin: current-discovery`，只引用原始业务资料、人工指定资料或已确认父级方向，并提供完整 Web 保留/拆分方案；`unresolved_boundary` 必须写当前单元自己的边界张力，不能在兄弟之间复制模板。归组不等于”已经不能继续拆”。低延迟、共享存储、顺序处理、同一阶段、同一页面、部署便利、代码仓库或团队只是交付信息，不能决定项目边界；但同一业务数据责任若完整拥有多来源接收、标准化、质量与版本/血缘、留存和可信交付，可以形成数据边界，依据是完整业务生命周期而不是数据库产品。

schema v6 的每个 Outline 单元还必须携带完整的 `project_boundary` 项目边界合同，明确自有责任、范围、自有业务对象、非目标、上游输入、下游输出、独立验收和未决边界。自有业务对象必须与该单元能力原子实际使用的对象完全一致，不能夹带无关对象。上下游合同要覆盖该单元全部业务链，并由对应业务链的来源支持。每个非根子单元，包括 terminal，还必须用 `project_candidate_basis` 证明自己是长期存在、可独立规划验收且与兄弟粒度可比的项目，而不是功能步骤或父级主题桶；只有 frontier 列出下一层拆分方向，terminal 不能靠状态标签绕过这项门禁。`grouping_basis` 只证明多个能力原子为什么暂时放在一起，不能代替“这本身是一个什么项目”的定义；只写得出抽象共同目标，却写不出具体责任、排除项、独立验收和输入输出合同的归组，不能进入候选项目树。模型补出的完整边界可以作为 `ai-proposed` 候选进入 Web 审核，但在人工确认前不能成为基线。

拆分是必做分析，不是数量指标；不设固定子单元数量、偏好区间、一个原子一个项目的映射或整棵树的最大深度，也不要求越细越好。先用来源/人工权威和单一责任门禁排除跨责任宽桶，再在可采用方案中比较总复杂度：项目内部共同维护的状态、规则、角色和验收，加上项目之间的交接、重复状态、验收割裂与重复上下文。拆开后总复杂度更低且存在真实业务 handoff 时继续拆；保持一个有界责任的总复杂度更低时保留。只有单元目标不可再分或另一拆分会增加总复杂度，才标记为 `terminal`。禁止按前端、后端、数据库、API、适配器、引擎、消息队列、代码仓库、部署单元或团队拆分。

`000-*` 是唯一树根，也是一个有目标、范围、结果和来源的顶级 Outline 单元。它的第一次 `decompose` 固定只生成一个后代层级，不得在同一轮顺带生成孙级单元。普通单元每轮由模型选择生成两层或三层；若所有分支都已满足终止条件，可以提前结束。每轮 `decomposition_window` 必须记录展开根、完整父路径、实际生成深度、每个单元的树深和状态，以及 frontier/terminal 清单。界面三层限制只控制本轮显示，不得驱动业务合并、遗漏或截断。

`decompose` 中的业务分图除结构根外，所有业务节点都必须登记为本轮 `decomposition_window.units` 中的 Outline 单元。未登记的目标、能力、验收和其他详细说明不能伪装成 `01.1` 之类的项目层级；只有已确认 terminal 单元进入 `detail` 窗口后才能生成这些细节。

第三阶段在写入 Discovery JSON 前执行语义质量门，逐项检查来源覆盖、父子完整且不重叠、单元职责纯度、通过显式业务契约实现的松散耦合、完整业务生命周期、来源术语保真、终止依据和内部推理不外泄。写出前还要清理标题、摘要、节点、候选、推荐和问题中的方法论套话与实现分层词，并执行跨领域替换测试。若只替换产品名就能原样搬到无关行业，必须改成有来源的领域对象、动作、控制、结果、交接或明确证据缺口。第一次失败时重新提取或分组；再次失败时停留在 `explore`，把缺少的证据、结果、责任边界或交接点交给用户判断。

`frame` 只能从已正式确认的 `terminal` 单元开始，并且不再生成新的项目层级。它根据 PRD、完整父路径和已确认终止依据补齐详细功能 Outline；至少需要一条由用户确认或正式文档支持的完整业务链。只有模型候选，或缺少对象、动作、结果、来源时，继续停留在 `explore` 或形成 evidence gap，不能伪装成详细框定已经完成。

Constitution 与 PRD 分开读取、分开负责。`/sp.prd` 可以把当前 feature 明确适用的条款放进 `constitution_snapshot`，固定为 `read_only` 和 `governance_only`，作为独立的只读治理快照展示；没有文件或没有适用条款时直接展示缺失状态。Constitution 不参与 `business_context` 推断，不作为业务链证据、候选推荐来源或 discovery 问题，不写回 PRD，也不能成为 delta 目标。移除这份快照后，业务能力导图仍应依据 PRD 和业务来源独立成立。

`explore` 和 `frame` 使用 `discovery` 收集当前窗口的结构或详细框定意图，`specify_ready` 才使用现有 digest-bound `confirmation`。Discovery 以 XMind 风格导图呈现当前展开根、最多三层的本轮后代和全局约束；父路径只作为上下文显示，不重复生成。政策、合规、审计、权限、安全等跨分支规则单列到全局约束图并关联受影响节点。Constitution 仍在独立快照中只读展示，不混入业务约束。

每张图和每个节点都要有稳定 ID；只有真实未决边界才生成问题，问题、候选和用户输入必须绑定到具体项目边界或分析节点，用户点击节点后只处理该分支的判断。每个高价值问题提供 2-4 个候选、一个推荐及理由、“以上都不适用”和自由输入，并要求用户明确选择操作：`confirm_candidate`、`add`、`replace`、`exclude` 或 `context_note`。针对候选子项目的有效选项应是保持当前直接边界、与指定相邻项目合并并重述职责、进一步拆分明确职责、延期或排除，不能针对每个能力原子机械地生成一次保留/合并题。每个候选（包括推荐项）必须通过 `business_chain_refs` 和 `capability_atom_refs` 引用当前项目的完整归属集合；没有来源和业务链依据的推荐无效。任意树深的关键项目边界和能力归属判断都必须让用户深度参与；`specify_ready` 只能整理终端单元的已确认内容并检查产品结构完整性，不能从 Constitution 推断业务事实，也不能把尚未确认的上游选择埋进自动生成的详细 Outline。

Discovery 只收集当前窗口的分解或详细框定意图。用户确认后，父级记录 `Project Decomposition`，为每个直接子单元生成 `Subproject Handoff`，至少包含稳定身份、业务目标、整体结果、拥有能力和对象、范围/非目标、上下游交接、继承约束、来源和未决问题。handoff 不是子单元详细需求全集。`000` 保留顶级 Outline 语义、全局约束、子单元交接和跨单元影响记录，但不在同一轮替子单元生成更深层细节。普通子单元必须在自己的展开窗口重新读取资料并继续递归，直到正式确认 terminal 后才进入详细功能和实现准备。

`explore` 和 `frame` 中的候选边界问题必须留在 `outline-discovery-data.json` 及其图形确认页里，不能仅因为拆分未确认、风险高或状态是 `SPLIT_REQUIRED`/`NEEDS_DECISION` 就改走 `/sp.clarify`。生成或刷新 Discovery 后，下一步是让用户在图中确认；页面通过受限本地服务把响应机械写入 `specs/<feature>/prd/review/outline-discovery-response-pending.json`，然后用户回到 Codex 重新执行 `/sp.prd` 消费。写回过程不调用模型、不解释审核意见、不修改 PRD/Outline；下载响应仅在本地写回失败时作为降级。响应写入前后 `NEXT_COMMAND_EXEC` 都使用 `None`，直到下一次 `/sp.prd` 验证并消费响应。只有现有节点绑定 Discovery 合同确实无法表达的独立聚焦决策，才交给 `/sp.clarify`。

导图必须主动控制并均匀分配信息密度：单图最多 18 个可见节点、当前窗口最多 3 层；当单图达到 8 个节点时，任一层不得承载超过 60% 的节点。超出预算时应使用分图和下一轮展开，不能通过减少候选、合并能力原子或截断事实来勉强展示。三层是显示窗口，不是功能树上限。页面主操作固定写入 pending 响应；页面完成、本地写回或下载都不能授权 `/sp.specify`。

Discovery 的生成数据、用户响应、意图账本和正式确认必须分离：`specs/<feature>/prd/review/outline-discovery-data.json` 是可刷新候选，`outline-discovery-response-pending.json` 是页面机械写入、等待下一次命令消费的非授权响应，`outline-discovery-response-*.json` 是失败降级文件，`specs/<feature>/prd/review/outline-intent-ledger.json` 是逻辑追加式决策账本，`specs/<feature>/prd/review/outline-review-data.json` 只属于终端单元完成详细框定后的正式确认。每次 `/sp.prd` 启动先检查 pending 响应，校验 feature、response、candidate、target、operation、schema version 和重复 delta，再追加账本；未知版本必须阻断，不能猜测、静默升级或降级成不兼容的旧合同。不能让 discovery 和 confirmation 数据包互相消费。从账本生成正式 review data 是 `/sp.prd` 的单向编译步骤：模型读取已接受账本状态以及当前 PRD/Outline，产生新的正式确认产物；不是 confirmation consumer 读取 discovery package。

有效 delta 写回 PRD/Outline 时，用户直接输入标为 `[src:user]`，接受模型候选标为 `[src:user-confirmed]`，未接受候选保持 `[src:ai-proposed]`。每条已消费 delta 都要有稳定的 `<!-- intent-delta:<id> -->` 锚点；可能被替换或排除的现有条目使用 `<!-- intent-target:<id> -->`，对应的新 delta block 使用 `<!-- intent-ref:<delta-id>:<target-or-candidate-id> -->` 精确记录引用。模型先生成 `prd.md.tmp` 和 `spec-outline.md.tmp`，再运行 `node .specify/review/scripts/apply-outline-discovery.mjs --response <response-package> --prd-temp specs/<feature>/prd.md.tmp --outline-temp specs/<feature>/spec-outline.md.tmp`。helper 先校验响应、当前 Discovery 数据、候选、目标与已有账本；有效的新事件先追加到 append-only 账本，再检查临时文档的 delta ID、唯一来源标签、目标章节、精确值、引用关系、成熟度和非授权边界。临时文档验证失败时，正式 PRD/Outline 保持不变，已追加事件和 pending 响应都保留；修正临时文档后重试同一响应，不得重复追加事件。只有 helper 成功且 `intent-delta` 锚点在当前正式 PRD 中恰好出现一次时，事件才算 consumed，随后 `/sp.prd` 才能把 pending 响应原子移动到 `prd/review/history/consumed/`；此后同一响应重放必须拒绝。helper 使用 `specs/<feature>/prd/review/.outline-discovery-writeback.lock` 串行化同一 feature 的写回：锁仍由 active process 持有时必须停止并等待；只有先独占 `.outline-discovery-writeback.recovery.lock`、再复核主锁仍是刚才观察到的旧身份，才能回收已退出进程留下的 stale lock。主锁和恢复声明都有唯一所有权 ID，释放时发现换主必须保留后来者的锁。如果死进程同时遗留两个锁，helper 必须失败关闭并保留现场；操作者确认没有写回进程后只删除恢复声明再重试。旧主锁已经不存在时，helper 只能在取得新主锁后自动清理孤立恢复声明。validator/helper 缺失、崩溃、返回无效结果、delta 缺失、重复或错配时都必须失败关闭，不覆盖最后一份有效文档。连续两次临时文档验证失败且没有新证据时，停止重复生成，将阻断状态设为 `NEEDS_DECISION`，保留失败 delta 与诊断，并让用户用新的 correction 或 supersede 事件解决冲突；成功接受 correction、reversal 或其他新证据后，下一次生成前清零连续失败计数。撤销、替换或修正必须用新事件的 `supersedes_delta_id` 引用更早且已被当前正式 PRD 消费的事件，不能引用只写入账本但仍为 pending 的事件；旧事件继续保留用于审计，不能原地改写历史事件；缺失或重复的正式 PRD 锚点、前向引用和成环引用必须拒绝。

语义就绪后，`/sp.prd` 还必须生成 `specs/<feature>/prd/review/outline-review-data.json`，用共享 renderer 展示“产品意图、范围/覆盖、readiness”三个有边界的视图，并通过 `node .specify/review/scripts/serve-review.mjs --outline <feature>` 启动本地确认页。Review Data ID 必须在完整 JSON 写入后由 `.specify/review/scripts/review-data-id.mjs` 计算；它覆盖完整 review 合同，不能由模型编造，也不能在修改 review 字段后沿用。`spec-outline.md` 的 `Outline Confirmation` 块必须记录 Review Data ID、`Outline Digest`、`Source Authority IDs` 和 `specs/<feature>/prd/review/outline-confirmation.md` 路径。页面只负责帮助人判断并机械写回确认记录；浏览器状态、localStorage、页面完成态、本地写回或 download 本身都不构成授权。用户回到 Codex 重跑 `/sp.prd` 后，命令校验当前 Review Data ID、Outline Digest 和 authority IDs，才由模型处理定向修订或确认推进。已初始化项目不会自动得到新增模板；如果缺 discovery schema、renderer 模块或 launcher 支持，应先在目标项目运行 `specify init --force` 刷新模板，再重试 `/sp.prd`。

下载的确认包必须由 `/sp.prd` 消费并写成可进 Git 的 `outline-confirmation.md`。门禁必须从当前完整 `outline-review-data.json` 重算 identity；只有重算值、canonical digest、source authority 与 Outline 元数据及确认文档完全一致，且 needs-decision、unresolved、draft-excluded、revision-request 都为空时，才可把 `Outline Decision` 改成 `READY_FOR_SPECIFY`。任何 review 字段、`Outline Digest` 或 `Source Authority IDs` 的修改都会使旧确认过期，状态必须回到 `AWAITING_OUTLINE_CONFIRMATION` 并重新确认，不能由模型修改证据字段来“对齐”。
...
`/sp.specify` 消费 outline 前必须做轻量 freshness/source snapshot 检查和新确认合同的硬门禁。检查对象是 outline 的 `Based On`、`Source Snapshot` 或 `Source Authority Summary`、`Status History`、`Outline Decision`、`Handoff To Specify`、`outline-review-data.json` 和 `outline-confirmation.md` 是否仍匹配当前 `prd.md`、source authority、feature 边界和人工决策记录；不要用文件 `mtime` 或原始 hash 做硬门禁。Outline 确认使用专门定义的 canonical digest 合同，不把任意文件 hash 当作 freshness 代理。轻量脚本发现 `READY_FOR_SPECIFY` outline 缺少 `Based On`，或缺少 `Source Snapshot`、`Source Authority Summary`、`Evidence Signature` 三者之一时，只能输出 `WARN`，表示下游缺少稳定入口凭证，不代表业务语义已经失败；但新合同缺确认、identity/digest/source authority 不一致或仍有未决项必须阻断并回到 `/sp.prd`。旧 `READY_FOR_SPECIFY` outline 在一个 minor release 兼容窗口内只告警，下一次 `/sp.prd` 刷新必须升级到图形确认合同。
...
Flow/UI 确认页由可复用的 `speccompass-review-data` 工具链驱动：普通 `/sp.flow`、`/sp.ui` 只填结构化 review data / structured review data，不得修改 renderer，也不得为确认页编写 HTML/CSS/JS。review data 是待审内容 / review data is draft review content；确认页不是编辑器 / not an editor，不直接修改 flow 或 UI 设计。审核人先在浏览器保存选择和自然语言修改意见，再点击“写入项目”。受限 loopback writer 使用每进程 capability token，重读当前 review data 并校验完整 identity，只能把原始决定和意见机械写到当前 feature 的固定确认文档；它不调用模型、不解释意见、不生成或修改 Flow/UI。完全确认且没有 revision/open items 时，writer 返回 `--consume-review-confirmation`，所属命令只重验仍在 active 位置的当前确认并推进 readiness，不清除或重生成。有修改或未决项时，writer 返回普通所属命令，命令询问“保留记录重新审核”或“全部清除重新生成”，再重生成完整适用范围；保留记录只是非权威候选，清除后不得继承。revision request 是重新考虑的输入，不是新产物授权。renderer directory / renderer 目录仍是多文件固定基础设施，普通命令不得修改 HTML、CSS、JavaScript、写回、fallback、localStorage 或右侧确认栏状态机。固定 renderer 的短参数 URL 和 schema/validator 规则保持不变。Flow/UI review data 必须分别通过对应 schema 和 `.specify/review/scripts/validate-review-data.mjs`；校验失败不能收尾，不能提升 readiness。

当用户明确表示已经人工查看，或明确要求采用当前所有推荐项时，统一使用 `/sp.accept outline|flow|ui <feature> [--advance]`。该命令在目标锁内重读当前 review data，运行正式 validator，逐项采用 `recommended_option`，然后调用同一机械 writer 原子写入固定确认文档；确认记录包含 `authorization_source: explicit_recommended_command`、接受数量和 critical 数量。它不接受缺失推荐、`needs-decision` 出口、过期 identity 或 Outline boundary adjustment/adoption，且不会把浏览器状态当授权。默认只保存确认；只有 `--advance` 才按非根 `outline -> specify`、`flow -> ui`、`ui -> gate` 顺序先消费当前确认再调度下一阶段。`000-*` 顶层组合根的 Outline 确认消费后必须调度 `/sp.route all` 选择明确的 `001+` 实施子项目，绝不运行 `/sp.specify 000-*`；消费或下一阶段失败即停止。

renderer directory / renderer 目录仍是多文件固定基础设施。固定 renderer 的短参数入口是 `.specify/review/renderer/speccompass-review-renderer.html?flow=<feature>` 和 `.specify/review/renderer/speccompass-review-renderer.html?ui=<feature>`；批次 Markdown 只是失败时的文本兜底。交互复核必须在项目根目录启动固定服务：flow 使用 `node .specify/review/scripts/serve-review.mjs --flow <feature>`，UI 使用 `node .specify/review/scripts/serve-review.mjs --ui <feature>`。启动器默认绑定 `127.0.0.1`；需要同一内网或 Tailscale 网络的其他设备访问时，可显式追加 `--host <RFC1918-or-Tailscale-ip>`（例如 `--host 10.0.0.209` 或 `--host 100.64.0.5`）。允许的非回环地址仅为 RFC1918 私网 IPv4 和 Tailscale CGNAT 的 `100.64.0.0/10`；公网地址、域名和 `0.0.0.0` 会被拒绝。只有在 renderer 和 review data 均返回 HTTP 200 后才输出 `SPECCOMPASS_REVIEW_URL=`；模型必须保持该进程运行并把原样 URL 交给用户，不能猜测或替换端口。内网/Tailscale 模式会打印警告，审核完成后应停止服务，因为同一网络中可达设备可能读取当前复核数据并尝试提交本次写回；静态访问白名单只包含固定 renderer、需求导航使用的同类型结构化 review data 和 `specs/review-index.json`，不提供其他项目文件。交互复核禁止使用 `file://`，并且 `localhost` 不接受；HTTPS、`::1` 和其他主机名同样不接受。已有项目如缺少这套固定基础设施，使用 `specify init --force` 刷新 `.specify/review/`；`specs/` 下的项目 review data 和确认文档不属于该固定目录。
...
Flow/UI 复核页还必须维护 schema v2 轻量需求索引 `specs/review-index.json`。已登记时，根需求下的 `outline-boundaries.json` 是项目身份、代码、顺序、父子关系和 Outline 映射的唯一可写事实源；普通命令只直接变更自己的 availability flag（`has_flow_review`、`has_ui_review`、`has_outline_review` 或 `has_outline_discovery`）与 `updated_at`，再用 `sync-review-index.mjs` 重建派生字段并校验。尚未登记时，`--intent regenerate` 返回非阻断提示：命令在显式 feature 内生成，保留 index 身份字段，只在现有且无需改 identity 仍能通过校验的 schema-v2 entry 上更新自己的 flag，不猜 parent、顺序或映射。遗留 index 无法校验时保持原样并报告非阻断导航提示，不得借机 sync、migrate、adopt 或阻断业务产物。真实 active transition、损坏权威或已登记 target mismatch 才阻断。固定 renderer 的导航和未写回提醒规则保持不变。

存量项目缺少 `outline-boundaries.json` 时，普通 PRD/Flow/UI 不再进入 bootstrap 循环。共享门禁的 regeneration intent 返回 `allowed: true` 和 `UNREGISTERED` 提示，主命令先生成真实业务产物。旧 `--adopt-outline-boundaries`、bootstrap、ADOPTION 页面和 activator 仅用于用户显式维护或恢复既有 receipt；不得作为普通命令唯一下一步。schema-v1 flat parent/sibling 仍表示未知，不能写成用户已确认。旧边界 candidate 可以在本轮生成完成后询问是否清理重建，但不形成额外审核阶段。

普通 `/sp.prd`、`/sp.flow`、`/sp.ui` 统一先运行 `reset-command-artifacts.mjs inspect`。这里的“普通”指直接执行完整阶段，不包括确认消费、结构迁移影响/恢复和 coordinator 明确授权的 Lite 小范围轮次。所有只读 prerequisite、Lite、hook、边界和路由检查先完成；任何检查阻断或缩小范围时，不得先删 active 产物。无人工记录时直接按 inventory digest 清除本命令旧产物；有确认、待消费写回或人工意图时，用户选择 `preserve-confirmed` 或 `clear --ack-confirmed`。保留记录写入 `review/history/regeneration-preserved/<inventory>/` 并标为非权威复审输入；全部清除后模型不得引用旧决定。PRD 只清 Outline 产物并保留 `prd.md`；Flow/UI 只清自己的目录；任何模式都不删代码。旧 `discard-outline-draft` plan/receipt 和专用 regenerate 命令只保留作兼容恢复，不再是普通入口。
...
Flow/UI 确认结果的主出口是“写入项目”。受限本地 writer 只接受 launcher 当前会话的 capability token，重读并验证当前 review data，按 review type 和 feature 推导唯一目标，原子写入 `flow-confirmation.md` 或 `ui-confirmation.md`；客户端不能指定任意路径。writer 只做机械记录，不调用模型。launcher 要求 Node.js 18 或更高版本；浏览器同一时间只允许一个写入请求，使用稳定 request ID 和有界超时/重试，服务端按目标文件跨进程串行，在锁内校验目标 SHA-256 版本，幂等复用已完成请求，并对临时文件执行 fsync 后原子替换。Windows 的 `EPERM`、`EACCES`、`EBUSY` 只做有界重试。只有服务端明确返回 `allow_fallback: true` 的网络、超时、临时文件系统或存储失败才显示独立下载链接；Review Data 陈旧、目标文件已变化、包无效、权限失败或 request ID 内容冲突都必须 fail closed，不显示下载/复制兜底，而是刷新或修复后重审。失败后主按钮始终是“重试写入”，审核选择变化会作废旧链接和旧请求。`speccompass-confirmation-package` JSON 同时作为 writer payload 和合格写回失败后的下载降级；复制摘要是最后降级。超过 `100000` UTF-8 bytes 时仍按完整 record 分包，每包重复 `package_session_id`、`review_type`、`batch_id`、`review_data_id`、`source_review_data`、`target_path`、`part_index`、`part_count`、`total_record_count`、`part_record_count`、`continuation_from`、`continuation_to`、`package_instruction`、`module_context` 和每条 record 的 `module_id` / `module_title`。多包消费前必须验证数量、同一 session/identity/target、相同 `total_record_count` 及 `sum(part_record_count) == total_record_count`，再按 `part_index` 合并；不得用单个 part 覆盖目标。DRAFT / `draft_excluded_items` 始终未授权。浏览器状态、机械写回、下载、复制和截图本身都不是后续产物授权。完全确认且没有修改或未决项时，writer 返回所属命令的 `--consume-review-confirmation` 入口，命令只复核当前 active confirmation 并推进 readiness；有修改或未决项时才返回普通所属命令，进入“保留记录复审 / 清除全部旧产物”二选一并完整重生成。

UI review data is not flow review data / UI 审核数据不是 flow 审核数据。每个 `screens[]` 条目必须先用 `business_context`、`primary_users`、`entry_scenarios`、`user_goal` 和 `user_outcome` 说清这个页面在整体业务中的位置、谁在什么情况下进入、要完成什么任务、完成后得到什么结果，再用 `screen_layout`、`screen_regions` 和可见 `components` 描述界面；`flow_refs` 只保存来源证据。Flow 中的角色、事件、状态、异常、权限和业务结果可以作为 UI 设计输入，但 Flow node ID、edge、branch、Mermaid 图或阶段进度不能直接成为产品 UI，除非 PRD 明确要求流程监控功能。`用于展示相关信息`、`方便用户查看数据`、`列表加详情`、`顶部加侧栏` 等布局或通用话术不能满足 Screen 语义契约。可选 `states` 只补充屏幕状态说明；`nodes` 只用于右侧确认栏的决策和授权模型，不能用一组 flow 节点/边冒充界面。屏幕布局 / screen layout 要说明这是表单、看板、列表详情、向导、详情、设置、页面地图、弹窗或自定义结构；每个区域要写清位置、用途和组件；按钮、输入框、表格、卡片、导航、空态、错误提示、徽标、图表说明等都要作为结构化组件出现。确实重要的动态行为使用 dynamic marker / 动态标注或纯文本标注，例如“此处数字未来会自动更新”，不得写动画、弹窗实现或 renderer 指令。决策选项需要深度推理 / decision options require deeper reasoning：每个需要人工判断的 UI 节点都必须用人话说明真实界面或交互背景，节点级必须提供 `背景信息` / `decision_background` 和 `决策摘要` / `decision_summary`；`must_confirm` 节点必须提供 3-4 个可执行选项，普通人工判断默认 3 项，低风险二元判断只有在 `options_count_rationale` 说明为什么 2 个出口足够时才允许 2 项。每个选项必须给出 `收益` / `benefit` 和 `代价` / `cost`，推荐项必须额外给出 `推荐理由` / `recommendation_reason`，让非技术 reviewer 能判断选择会解锁、延后或阻断哪些后续工作。`consequence` 和 `next_exit` 是写回与路由用的执行字段 / execution field，仍然必须存在：`consequence` 说明选择后模型或责任人要改什么，`next_exit` 给出可执行出口并点明谁继续处理。选项文案必须说明不选推荐的代价，并在同一节点内保持真实差异，不能只是把同一条路线改写成“保留 / 补充 / 局部调整 / 后续完善”。生成选项前必须先选决策模板：范围决策说明哪些 screen、区域、状态或 action 进入本批；门禁决策说明界面是否能发布、是否要补材料、是否要转人工审核；降级决策说明完整能力不可用时采用什么更简单的界面或人工兜底。常见出口必须按 `/sp.clarify` 的拍板包标准写：needs-decision 选项必须说清缺什么、谁拍板、哪些下游工作暂停；split-flow 选项必须说清拆成哪些子流程；推荐项必须说明为什么比更慢、更重或更保守的替代方案更适合。禁止复用模板句、stock phrase 或 boilerplate，例如“当前依据看起来正确”“后续完善相关内容”；同一节点内多个选项如果只是换编号但背景、收益、代价和推荐理由相同，也视为模型偷懒。面向 reviewer 的技术词必须改成业务语言，确实必须保留英文或术语时，要在同一句给出中文说明或解释。命令收尾前必须运行 `.specify/review/scripts/validate-review-data.mjs`，让校验器拦截 Screen 语义字段缺失、布局式或通用填充话术、重复选项文案、模板句、未解释技术词、空泛出口、缺失背景/摘要/收益/代价/推荐理由、缺失“谁继续处理”、必审节点缺少 why-now 说明，以及多个选项的收益或代价高度相似；这类失败属于模型可修质量问题，不能转成让用户人工兜底。

确认页页面代码是共享基础设施，不是每次命令生成物。普通 `/sp.flow`、`/sp.ui` 不得把 HTML、CSS、JavaScript、SVG、CSS class、事件处理器或页面布局指令写进 review data；自由文本备注只能写业务说明。确认页不追求复杂动画 / no complex animation；固定 native `<dialog>` only for explanation or preview，不承载选择、意见、授权、写回或 fallback 动作，并必须使用安全 DOM/text API 而不是 `innerHTML`。runtime validation 只做浏览器防护，命令收尾仍运行 `validate-review-data.mjs`。localStorage 草稿按完整 review identity 隔离，但不是授权。写回或 fallback 失败不能标记完成；存在未写入项目的选择时，离开、关闭或刷新前提醒。右侧路径必须显示“本地选择 → 写入项目 → 回到 Codex 执行 writer 返回的精确命令”，并把下载确认包和复制摘要明确标为失败降级，避免把机械记录误认为模型已经处理或新产物已经授权。
...
Flow/UI 确认页采用统一模板：顶部显示 `SpecCompass — <项目名> / <feature>`、简短 specCompass 机制说明和页面标题；左侧或中部展示带可见标签的 flow 图、流程表、UI 屏幕或项目 UI 全貌；右侧是一条约 280-320px 的窄确认栏，使用 Tiffany Blue `#0ABAB5` 作为主色，包含 batch 摘要、选中项详情、状态提示、意见输入、逐节点决策选项卡、逐节点反馈输入框、待决策列表、阻塞或 stale 列表和批量确认按钮。每个需要人工判断的节点必须提供分层可执行选项：Flow 的 `must_confirm` 使用 2-4 个可执行选项，来源明确且互斥的二元 Flow 判断必须用 `options_count_rationale` 说明为什么不存在第三条真实路径；UI 的 `must_confirm` 必须使用 3-4 个可执行选项，只有普通、非 `must_confirm` 的低风险二元 UI 判断才允许在说明理由后使用 2 项；普通人工判断默认 3 项。默认优先展示节点 `背景信息`、`决策摘要`，以及每个选项的 `收益`、`代价`，推荐项额外展示 `推荐理由`。选择后模型动作、后续出口、授权追溯和确认所选方案必须放入折叠详情或确认记录中；`consequence`、`next_exit` 作为执行字段 / execution field 继续用于写回和路由。不能把“通过 / 暂缓 / 退回 / 阻塞”作为节点级主按钮。右侧反馈确认栏是 flow 确认页的合格条件；缺失时，该 review artifact 不能作为授权证据，也不能支撑下游 `READY_FOR_UI` 或实现准入。确认页必须显示写回路径，并能让用户清楚知道确认结果需要进入 git 跟踪。
...
Flow 的 project business overview、review goal、module summary 和 per-flow summary 都必须是实质性的业务上下文，不能只写“展示流程”“处理相关业务”等泛化套话；校验器应在生成阶段拦截这些空话，避免 reviewer 只看懂局部句子却不知道流程在整体业务中的作用。
...
- 生成 Flow review-data 时，`plain_summary` 不能只重复节点名称或写“进入下一步”“系统处理数据”之类的泛化话术；必须让审核人看出触发场景、责任角色、业务动作、状态/结果变化，以及下一责任或异常恢复路径。`decision` / `human_judgment` 节点的每条出边都必须写明业务条件、判断结果或恢复原因，不能只写“继续”“下一步”“处理”“完成”。`must_confirm` 默认使用 3-4 个可执行选项；如果来源只支持两个互斥出口，可以使用 2 个，但必须提供 `options_count_rationale` 说明为什么不存在第三条真实路径，禁止为了凑数量虚构事实。
...
- flow 确认页的节点选项必须采用两步交互。情况 A：推荐选项点击即保存 / recommended-option click saves immediately，卡片先显示“正在保存推荐选择”一类即时可见反馈 / immediate visible feedback，再显示“已按推荐保存，可重新选择”和“重新选择”；状态区使用 `aria-live`。保存函数重新计算 `recommended_option` 并校验点击项仍为推荐项，否则降级为非推荐 DRAFT，不能绕过审核意见。提示语必须区分浏览器本地选择、机械写入确认文档、以及下一次 Codex 命令处理；本地选择和写回本身都不代表模型已经生成新产物。情况 B：非推荐选项 / non-recommended option 只进入“待提交草稿”，立即显示“非推荐选项已暂存为草稿”，打开并聚焦审核意见输入框；填写原因并点击邻近的“提交选择”后才保存。节点级动作只局部更新卡片、输入框、选中摘要和图上高亮，不触发 Mermaid 完整重绘、整页 `render()` 或整条右栏重建；统计可在局部反馈后延迟刷新。持久化 DRAFT 重新进入视图后自动展开意见区；空意见在输入框下方显示“请先填写审核意见”，不能只用弹窗或页级提示。状态机显式区分 `MISSING | DRAFT | SAVED_RECOMMENDED | SAVED_SUBMITTED`。重新选择清空正式选择和草稿，回到未选择。草稿不能进入 `decision_records`，不能被统计为已确认或支撑 readiness；未选择或没有出口路径进入 `unresolved_decision_items`，DRAFT 只能进入 `draft_excluded_items`，因为草稿不具备授权意义。“写入项目”前存在 DRAFT 时，第一次点击必须在按钮附近提示“草稿不具备授权意义，写入项目前请先处理草稿”，按钮改成“仍要写入项目”，不得重建右栏、丢失输入、重绘图或调用整页 `render()`；第二次明确继续才可机械写入，且 payload 只能把草稿列入 `draft_excluded_items` 并保留 draft warning。下载确认包降级和复制摘要降级执行相同草稿排除规则，不能把草稿变成授权。离开、关闭或刷新时使用 beforeunload / navigation/close 提醒。批量按推荐确认不能覆盖已有选择、已提交非推荐选择或 DRAFT；反馈说明保存数及跳过并保留的选择和草稿。重置只清除当前视图 localStorage 临时选择，并提示正式记录以项目确认文档为准。
...
Flow/UI 的确认摘要如果包含需要人工决策的点，必须按 `/sp.clarify` 风格的决策包说人话：先交代 `背景信息` 和 `决策摘要`，再按 review type 给出真实可执行选项。Flow 的 `must_confirm` 使用 2-4 项，来源明确且互斥的二元判断必须说明为什么不存在第三条真实路径；UI 的 `must_confirm` 必须使用 3-4 项，只有普通、非 `must_confirm` 的低风险二元 UI 判断才允许在说明理由后使用 2 项。每个选项说明 `收益` 和 `代价`，推荐项额外说明 `推荐理由`，同时保留 `consequence`、`next_exit` 作为执行字段 / execution field 让下一轮模型知道如何写回。不能只把问题抛给用户，也不能在用户确认前把 `DRAFT_ONLY` 提升成 `READY_FOR_UI`、`READY_FOR_PLAN` 或 gate/plan/tasks 的准入证据。
...
`migrate-review-index.mjs` 只保留作遗留诊断，不是 PRD/Flow/UI 重生成的前置条件；已登记项目运行 `validate-review-index.mjs`、`sync-review-index.mjs` 和 `check-outline-boundary-gate.mjs --intent regenerate --stage <prd|flow|ui>` 校验正式边界。所有带 `--feature` 的检查都必须显式传所属 stage，不能靠省略 stage 绕过根级职责限制。未登记项目只在现有 schema-v2 entry 能原样保留 identity 并通过校验时更新 availability flag；遗留 index 无法校验时保持原样、提示导航不可用，但不阻断主产物。
