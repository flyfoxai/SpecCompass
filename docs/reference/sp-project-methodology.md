...
需求入口也遵循这个原则。`/sp.prd` 是所有新 feature、能力方向和重要需求变更的强制上游入口，用于从 0 到 1 的需求发散、访谈式收集、候选需求隔离和 PRD 草稿沉淀。简单需求可以走精简 PRD，但不能跳过 PRD。精简 PRD 只适用于用户已经给出清楚目标、用户、范围和基本验收意图的情况，最少也要保留战略目标、目标用户、核心场景、范围/非目标、验收种子、风险/open questions 和 handoff；0 到 1 想法、范围不清、多能力方向、治理影响、高风险或 source 冲突必须走完整 PRD。`prd.md` 是需求来源容器：用户确认、正式来源或已消费人工决定支持的内容是 Outline 不得丢失的需求事实；`[src:ai-proposed]`、`[uncertain:*]` 等明确候选不是事实。`/sp.specify` 仍是稳定规格入口，负责把已确认、可追踪、适合稳定化的内容写入 `spec.md`。如果执行 `/sp.specify` 时缺少 `prd.md` 或 PRD readiness，应先返回 `/sp.prd`。
...
二级只负责单个已确认 `001+` 实施子项目的业务闭环，不重新做全局分组。无论一级确认一个还是多个实施产品，二级都只读取一份已确认 `Subproject Handoff`、该交接引用的来源、继承约束和子项目局部变更；只有一个实施子项目时，这份 handoff 仍是真实的职责移交合同，用来阻止 `000` 被隐式当成实施叶子。二级必须把首个切片写成具体业务链：触发者或输入、业务对象及起始状态、动作或控制、结果状态或可观察结果、异常路径、来源和命名交接。三级只负责保留来源身份的正式编译：把已确认 PRD、已消费账本事件、二级框架和来源权威编译为 `spec-outline.md` 与 `outline-review-data.json`，不能新建、合并、拆分或重新解释业务事实，也不能提升未确认模型建议。Constitution 在三级仍只读展示；正式治理判断由 `/sp.constitution` 负责。成熟度只回退到受影响的最窄层级：单个交接或两个已确认子项目之间的能力归属冲突只重开受影响的命名边界，其余子项目保持稳定，二级不能静默合并子项目；只有一个或多个实施边界的决定被推翻，或整个产品组合身份、所有权模型被否定，才重开完整一级拆分。
...
第二阶段先平铺全部原子，再逐一编译候选子项目和业务导图。初始生成严格执行“一个能力原子、一条业务链、一个候选子项目”，即使责任顺序相邻、共享业务对象或存储、运行在同一进程，或共同服务于后续终局结果，也不能预先合并。合并只能作为用户选择：绑定到具体候选分支，写明要并入的相邻项目及合并后的状态、结果和交接，并在有效 Discovery 响应确认后才生效。每个候选子项目必须有单一业务目标、可独立核验的业务结果、内聚的业务对象/规则/状态，以及与相邻项目之间少量且明确的业务输入、输出、命令或事件。拆分是必做分析，不是数量指标；不设固定子项目数量。低延迟、共享存储、顺序处理或部署便利只是交付风险，不能单独否定产品拆分。事务一致性或双向业务交接必须先分类：若由法规、合同或多方法律义务强制，则保留为业务不变量；否则只记为交付风险。禁止按前端、后端、数据库、API、适配器、引擎、消息队列、代码仓库、部署单元或团队拆分。候选项目标题若只靠“中心、枢纽、平台、系统、模块、工作台、网络、接口、适配器、数据库、队列、界面、报表”等容器词或实现词成立，应继续检查它拥有的业务状态、独立结果和交接契约；前面加上领域词也不能补救缺失的业务边界。Overview 用真实产品组合、业务系统或受控业务闭环命名，每个一级入口对应一个原子候选子项目。
...
一级和二级使用 `discovery`，三级才使用现有 digest-bound `confirmation`。Discovery 以 XMind 风格思维导图呈现当前产品全局，不再把长篇 Outline 或平铺问题列表当作主入口。每轮至少包含三类图：一张全局总图、一张或多张候选子项目业务分图、一张全局约束图。总图展示真实产品及候选项目边界并提供分图入口；子项目分图承载业务目标、拥有的能力与对象、业务场景、范围、结果和交接；政策、合规、审计、权限、安全等影响多个候选项目的业务规则单列到全局约束图，并明确关联受影响的业务节点。Constitution 仍在独立快照中只读展示，不混入全局业务约束；只影响单一候选项目的规则留在该分图。

每张图和每个节点都要有稳定 ID；问题、候选和用户输入必须绑定到具体节点，用户点击节点后只处理该分支的判断。每个高价值问题提供 2-4 个候选、一个推荐及理由、“以上都不适用”和自由输入，并要求用户明确选择操作：`confirm_candidate`、`add`、`replace`、`exclude` 或 `context_note`。针对候选子项目的有效选项应是保持独立、与指定相邻项目合并并重述职责、进一步拆分指定职责、延期或排除，不能为了凑数生成机械的保留/删除题。每个候选（包括推荐项）必须通过 `business_chain_refs` 引用真实业务链；没有业务链依据的推荐无效。一级和二级的关键产品判断必须让用户深度参与；三级只能整理已确认内容并检查产品结构完整性，不能从 Constitution 推断业务事实，也不能把尚未确认的一级、二级选择埋进自动生成的三级细节。

Discovery 只收集拆分意图。用户通过有效响应确认边界后，父级 PRD 记录 `Project Decomposition`，并为每个独立子项目生成一份子项目交接（`Subproject Handoff`），至少包含稳定 ID、单一业务目标、目标角色、拥有的能力和业务对象、范围/非目标、上游输入、下游输出、继承的全局业务门禁、来源和未决问题。Level 1 不自动创建 feature 目录、不决定实施顺序，也不授权 `/sp.specify`。`000` 永远是纯 Portfolio/统筹根，只能保留 PRD、Outline、全局约束、子项目交接、跨项目依赖和影响记录；它不能拥有或消费 `spec.md`、Flow、UI、Bundle、Plan、Tasks、analysis、gate 或实现产物。任何统一壳、组合控制台或跨项目业务旅程，都必须先分配给一个 `001+` 的可验收实施子项目。已确认拆分的父级项目不能继续作为一个整体进入 `/sp.specify`；每个子项目应分别建立 feature 目录，完成自己的 `/sp.prd` 和 Outline 确认。若用户确认保留为单项目，也不能把 `000` 变成实施 feature；应创建至少一个 `001+` 实施边界，或继续停留在 Outline 决策阶段。
