...
每张图和每个节点都要有稳定 ID；问题、候选和用户输入必须绑定到具体节点，用户点击节点后只处理该分支的判断。每个高价值问题提供 2-4 个候选、一个推荐及理由、“以上都不适用”和自由输入，并要求用户明确选择操作：`confirm_candidate`、`add`、`replace`、`exclude` 或 `context_note`。一级和二级的关键产品判断必须让用户深度参与；三级只能整理已确认内容并按 Constitution 检查结构和治理覆盖，不能凭空补业务事实，也不能把尚未确认的一级、二级选择埋进自动生成的三级细节。
