...
当前单元的 Outline 生成先读取父级 handoff 和继承约束，再重新扫描默认 `prd/`、人工指定资料目录、当前 feature PRD、父级引用和本地已确认资料。handoff 是方向和责任合同，不是完整需求白名单。父子核对检查方向、范围、责任、结果、交接、来源和全局约束的闭合关系，不比较两边是否写出相同节点或数量。只有正式确认的终端单元才进入详细功能框定；资料不足但可以设计的结构标记 `ai-proposed`，缺失的业务事实保留 evidence gap 或进入所属 Web 决策页。
...
每个 Outline 单元必须有自己的业务目标、可核验整体结果、内聚的业务对象/规则/状态和来源；父单元不能因为已经拆出孩子而变成空白目录。单元引用的业务链集合必须等于其能力原子引用业务链的集合。多原子单元写出机器可读 `grouping_basis`，说明共同业务目标、共同生命周期或责任主体，以及拆开后具体会增加什么协调、重复状态或验收损失。PRD 经常只给业务方向和事实而不直接规定项目架构，因此 `grouping_basis.authority` 可以是 `ai-proposed`，但生成结果仍只是等待 Discovery 确认的候选；`unresolved` 不能支撑已经归组的非根单元，必须把具名划分方案交给用户判断。归组不等于“已经不能继续拆”。是否继续拆由 `decomposition_basis` 判断，是否到达末端由 `terminal_basis` 判断。低延迟、共享存储、顺序处理、同一阶段、同一页面、部署便利、代码仓库或团队只是交付信息，不能单独决定项目边界。
