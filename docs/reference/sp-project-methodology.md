...
第二阶段先平铺当前根单元的全部原子和一对一业务链，再按业务责任递归分配给直接子单元。固定关系是：“一个来源能力形成一个能力原子；一个能力原子只归属一个直接子单元；具有独立状态、独立结果或明确 handoff 的能力默认形成独立候选边界；父单元只有在聚合依据成立时才可以包含多个能力原子和业务链”。这是面向当前模型能力的复杂度控制：默认拆分、显式交接、单一业务职责、松散耦合；合并是有依据的例外。能力原子不是自动创建项目的数量配额，但模型不能仅凭一个宽泛主题或阶段名称把多个原子装进同一大单元。

每个 Outline 单元必须有自己的业务目标、可核验整体结果、内聚的业务对象/规则/状态和来源；父单元不能因为已经拆出孩子而变成空白目录。单元引用的业务链集合必须等于其能力原子引用业务链的集合。多原子单元写出机器可读 `grouping_basis`，说明共同业务目标、共同生命周期或责任主体，以及这些能力为什么在当前父分支内保持内聚；它不等于“已经不能继续拆”。是否继续拆由 `decomposition_basis` 判断，是否到达末端由 `terminal_basis` 判断。低延迟、共享存储、顺序处理、同一阶段、同一页面、部署便利、代码仓库或团队只是交付信息，不能单独决定项目边界。
