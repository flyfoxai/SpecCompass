...
`Stage Readiness` 的推荐字段包括：`Stage`、`Status`、`Based On`、`Unresolved Blockers`、`Needs Decision`、`Inferred/Draft Items`、`Next Allowed Stage` 和 `Writeback Target`。它不是新台账，只是当前阶段产物对下一阶段的入口声明；长期 blocker 仍然写入 `memory/open-items.md`。
