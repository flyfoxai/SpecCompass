...
`migrate-review-index.mjs` 只保留作遗留诊断，不是 PRD/Flow/UI 重生成的前置条件；已登记项目运行 `validate-review-index.mjs`、`sync-review-index.mjs` 和 `check-outline-boundary-gate.mjs --intent regenerate` 校验正式边界。未登记项目只在现有 schema-v2 entry 能原样保留 identity 并通过校验时更新 availability flag；遗留 index 无法校验时保持原样、提示导航不可用，但不阻断主产物。
