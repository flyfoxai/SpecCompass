...
Flow/UI 确认页由可复用的 `speccompass-review-data` 工具链驱动：普通 `/sp.flow`、`/sp.ui` 只填结构化 review data / structured review data，不得修改 renderer，也不得为确认页编写 HTML/CSS/JS。review data 是待审内容 / review data is draft review content；确认页不是编辑器 / not an editor，不直接修改 flow 或 UI 设计。审核人先在浏览器保存选择和自然语言修改意见，再点击“写入项目”。受限 loopback writer 使用每进程 capability token，重读当前 review data 并校验完整 identity，只能把原始决定和意见机械写到当前 feature 的固定确认文档；它不调用模型、不解释意见、不生成或修改 Flow/UI。下载确认包只在本地写回失败时显示，复制摘要是最后降级。写入成功后，用户回到 Codex 重跑所属 `/sp.flow` 或 `/sp.ui`；命令校验 feature、review type、batch、Review Data ID 和 source identity 后，模型才读取 `revision_requests`。每个 `target_ref` 是修订边界：只重生成命中项和确实因共享契约失效的直接邻居，显式列出邻居，保留未受影响的已接受决定，并让下一轮审核只包含 changed/invalidated items。revision request 是修订指令，不是新产物的授权。Flow 修改类型包括 `ADD_NODE`、`DELETE_NODE`、`MODIFY_NODE`、`MODIFY_BRANCH`、`ADD_EXCEPTION_PATH`、`SPLIT_SUBFLOW`、`MERGE_SIMPLIFY`、`ADD_ENTRY_EXIT`、`OTHER`；UI 修改类型包括 `ADD_SCREEN`、`DELETE_SCREEN`、`MODIFY_SCREEN_STRUCTURE`、`ADD_REGION`、`MODIFY_REGION_LAYOUT`、`ADD_COMPONENT`、`DELETE_COMPONENT`、`MODIFY_FIELD_ACTION_COPY`、`ADD_STATE`、`MODIFY_INTERACTION`、`ADD_PERMISSION_DISPLAY`、`OTHER`。renderer directory / renderer 目录 `.specify/review/renderer/` 是 multi-file fixed infrastructure / 多文件固定基础设施；普通命令不得修改 HTML、CSS、JavaScript、写回、fallback、localStorage 或右侧确认栏状态机。固定 renderer 的短参数 URL 是 `.specify/review/renderer/speccompass-review-renderer.html?flow=<feature>` 和 `.specify/review/renderer/speccompass-review-renderer.html?ui=<feature>`；实际审核应使用 launcher 输出的准确 Web URL，`flow-review-batch.md`、`ui-review-batch.md` 仅为文本兜底。Flow/UI review data 必须分别通过对应 schema 和 `.specify/review/scripts/validate-review-data.mjs`；校验失败不能收尾，不能提升 readiness。
...
存量项目缺少 `outline-boundaries.json` 时，不得让普通 `/sp.prd` 与 bootstrap 命令互相跳转。共享门禁返回唯一的 `/sp.prd <root> --adopt-outline-boundaries`：机械 bootstrap 生成带 review-index digest 的候选，模型读取真实 PRD、Outline、目录与 handoff 生成 `ADOPTION` proposal 和正式审核页，用户通过 loopback writer 最终确认，机械 activator 再重验目录、代码、parent、Outline/handoff、产物摘要、writer ledger 和一次性 receipt。schema-v1 flat index 迁移使用 `parent_feature: null`、`sibling_order: 0` 表示非根条目的层级未知；采纳器只在候选同时带有 `parent_unconfirmed` 时允许 proposal 将这个占位顺序规范化为大于等于 1，已经明确的 parent 与 sibling 顺序仍禁止改写。激活器用短租约与 exclusive create 建立一个首个 `ALIGNED` baseline，初始化 feature-code ledger，派生 review index，并在崩溃或并发下幂等前滚。这个特殊模式只记录当前事实，不改写已有业务产物、不进行项目重构；激活并重新通过门禁后，普通 PRD/Flow/UI 工作才恢复。

如果用户明确拒绝采纳一份尚未确认的 Outline 草稿，可以在首个 `outline-boundaries.json` 建立前运行 `/sp.prd <root> --discard-outline-draft`。`discard-outline-draft.mjs plan` 只生成带摘要的精确归档清单；`apply` 只有在显式 plan digest、全部来源摘要、路径边界和短租约均有效时，才把 `spec-outline.md`、Outline review/discovery 数据、伴生确认/决定及未激活 proposal 移入 reset 历史目录，并最后写入 `outline-draft-reset.json`。它不删除 feature 目录，也不得归档 `prd.md`、`spec.md`、Flow、UI、Plan、Tasks、代码、测试、数据或 migration。中断后按同一 plan 幂等前滚；已有权威 baseline、active transition、symlink、越界路径、源/archive 双份或摘要漂移都 fail closed。

reset 后旧 review-index 条目只表示保留的 PRD/实现来源容器，不再约束新项目划分。共享门禁返回 receipt 绑定的 `/sp.prd <root> --regenerate-outline-draft --reset <reset-id>`，不能再走普通 legacy adoption。模型重新读取 receipt 列出的全部 PRD 和相关保留实现，生成一份全新的 root Outline；候选项目先使用 `draft-project-*` 临时身份，不携带稳定 code 或 active feature 声明。reset ID 与 receipt digest 必须进入 Outline source authority identity。人工确认新 Outline 后，才建立单独的项目 reconciliation proposal、分配候选 SP code，并用现有 inventory/staging/validation 机制处理每个保留产物。新 Outline、确认写回或 reconciliation 都不能自动删除代码。
