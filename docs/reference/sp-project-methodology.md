...
用户第一次进入 `/sp.prd` 时，材料可能还不足以直接形成稳定框架。此时用独立的 `outline_maturity` 表达 Outline 成熟度，而不是把缺口伪装成完整 PRD，也不能把它和确认点的 `review_level`、`confirmation_priority` 或整体 `Outline Decision` 混为一谈。固定三级是：`explore`（一级：方向探索）、`frame`（二级：框架收敛）、`specify_ready`（三级：完整大纲）。一级要求用户深度参与目标、用户和核心问题；至少确认目标、一个用户或角色和清晰核心问题后，才能进入二级。二级要求用户深度参与范围、非目标、首个交付切片、场景和高影响业务规则；这些边界稳定且来源足够明确后，才能进入三级。三级只在产品事实和边界足够稳定后，由模型整理已确认内容并检查产品结构完整性。Constitution 在 PRD 中始终只读展示，不参与推断、补全或治理裁决；正式治理判断仍由 `/sp.constitution` 负责。成熟度可以回退：目标、用户/角色或核心问题被撤回、替换或发生冲突时，`frame` 回到 `explore`；范围、非目标、首个切片、核心场景、高影响规则、验收意图或 source authority 实质变化时，`specify_ready` 回到 `frame`，一级最低证据也失效时继续回到 `explore`。

Level 1 采用两阶段生成。第一阶段先从用户输入、当前 PRD、已接受意图账本和正式业务来源中提取 `business_context`：真实业务主语 `product_subject`、业务对象 `business_objects`、业务动作 `operations`、可观察结果 `outcomes`、端到端 `business_chains` 和证据缺口。完整业务链必须同时说清触发或输入、被处理的对象、动作或控制、结果及来源；纯模型建议只能保留为候选。第二阶段才把完整业务链编译为导图：Overview 用真实产品或业务闭环命名，一级入口使用业务能力主干，说明产品具体储存、判断、控制、交易、交付或核验什么。目标、用户、问题、范围和“全局认知”只能作为支撑信息，不能冒充一级业务能力。
...
一级和二级使用 `discovery`，三级才使用现有 digest-bound `confirmation`。Discovery 以 XMind 风格思维导图呈现当前产品全局，不再把长篇 Outline 或平铺问题列表当作主入口。每轮至少包含三类图：一张全局总图、一张或多张业务分图、一张全局约束/治理图。总图负责建立项目全貌并提供分图入口；业务分图承载目标、角色、问题域、场景、能力、范围和验收种子；政策、合规、审计、权限、安全等影响多个业务分支的规则单列到全局约束图，并明确关联受影响的业务节点。只影响单一业务分支的规则仍留在该分图。

每张图和每个节点都要有稳定 ID；问题、候选和用户输入必须绑定到具体节点，用户点击节点后只处理该分支的判断。每个高价值问题提供 2-4 个候选、一个推荐及理由、“以上都不适用”和自由输入，并要求用户明确选择操作：`confirm_candidate`、`add`、`replace`、`exclude` 或 `context_note`。每个候选（包括推荐项）必须通过 `business_chain_refs` 引用真实业务链；没有业务链依据的推荐无效。一级和二级的关键产品判断必须让用户深度参与；三级只能整理已确认内容并检查产品结构完整性，不能从 Constitution 推断业务事实，也不能把尚未确认的一级、二级选择埋进自动生成的三级细节。

导图必须主动控制并均匀分配信息密度：单图最多 18 个可见节点、最多 3 层、每个节点最多 4 个直接子节点；当单图达到 8 个节点时，任一层不得承载超过 60% 的节点。超出预算时应重排层级或拆成新的业务分图，不能通过缩字、堆叠或把大部分信息集中到同一层来勉强展示。页面下载 `outline-discovery-response-*.json`，但不能授权 `/sp.specify`，也不能仅凭页面完成、localStorage 或下载动作把状态推进到 `AWAITING_OUTLINE_CONFIRMATION` 或 `READY_FOR_SPECIFY`。
