...
如果 `PROJECT_GOAL`、`PRIMARY_THEME` 或 `ROOT_BLOCKER_FAMILY` 无法可靠判断，必须停止并返回 `SP_STATUS: NEEDS_DECISION`，给出 2-4 个选择和推荐项。不能用“我先看某个 feature”替代方向判断。是否继续执行只看 JSON 的 `continueAllowed` 和 `autoExecute` 语义，不再新增第二套继续字段。
