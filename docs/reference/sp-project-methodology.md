...
一级和二级使用 `discovery`，三级才使用现有 digest-bound `confirmation`。Discovery 页面每个高价值问题提供 2-4 个候选、推荐及理由、“以上都不适用”和自由输入，并要求用户明确选择操作：`confirm_candidate`、`add`、`replace`、`exclude` 或 `context_note`。页面下载 `outline-discovery-response-*.json`，但不能授权 `/sp.specify`，也不能仅凭页面完成、localStorage 或下载动作把状态推进到 `AWAITING_OUTLINE_CONFIRMATION` 或 `READY_FOR_SPECIFY`。
