...
资料不足时应区分“模型能设计”和“必须由人决定”。不会改变范围、权限、安全、资金、合规、不可逆行为、关键验收或跨项目责任的低影响可逆选择，由模型记录来源与理由后继续。会造成重大业务影响或大范围返工的决定，必须放到所属 Outline、Flow 或 UI 的 SP Web 审核页，提供 2-4 个真实方案、影响和模型推荐；推荐不等于授权。三层确认后，Plan、Tasks 和 Implement 应更多依照它们形成的框架自行完成技术分解、任务和代码，只有发现上游冲突、新的重大决定或确认已经过期时才回退。
...
用户第一次进入 `/sp.prd` 时，材料可能还不足以直接形成稳定框架。此时用独立的 `outline_maturity` 表达 Outline 成熟度，而不是把缺口伪装成完整 PRD，也不能把它和确认点的 `review_level`、`confirmation_priority` 或整体 `Outline Decision` 混为一谈。固定三级是：`explore`（一级：项目组合边界）、`frame`（二级：单项目业务框架）、`specify_ready`（三级：正式 Outline 编译）。一级只负责项目组合拆分：判断 Portfolio 需要一个还是多个有独立业务结果、内聚责任和明确业务交接的实施产品；无论结果是一还是多，`000` 都只保留为统筹根，“保留单产品”表示建立一个明确的 `001+` 实施边界。不能仅凭“目标、用户、问题”三句套话过关。进入一级拆分前至少要有真实产品主语、一名执行领域动作的角色，以及正在失败或变化的业务对象、动作或结果。离开一级前，每个有来源的能力都必须有唯一归属或明确处置，每个候选边界都必须由有效用户响应确认、合并、再拆、延期、排除、分配给明确实施边界或标为证据缺口。

二级只负责单个已确认 `001+` 实施子项目的业务闭环，不重新做全局分组。无论一级确认一个还是多个实施产品，二级都消费一份已确认 `Subproject Handoff` 来约束方向、责任边界、继承约束和命名交接，同时重新扫描默认 `prd/`、人工指定资料目录、当前 feature PRD 和已接受的子项目局部变更。handoff 引用是优先来源，不是白名单，也不表示父级已经写全子项目细节。只有一个实施子项目时，这份 handoff 仍是真实的职责移交合同，用来阻止 `000` 被隐式当成实施叶子。二级必须从完整资料集合中把首个切片写成具体业务链：触发者或输入、业务对象及起始状态、动作或控制、结果状态或可观察结果、异常路径、来源和命名交接。父子核对只检查方向、范围、责任、结果、交接和全局约束是否一致，不比较两边是否生成相同节点。三级只负责保留来源身份的正式编译：把已确认 PRD、已消费账本事件、二级框架和来源权威编译为 `spec-outline.md` 与 `outline-review-data.json`，不能新建、合并、拆分或重新解释业务事实，也不能提升未确认模型建议。Constitution 在三级仍只读展示；正式治理判断由 `/sp.constitution` 负责。成熟度只回退到受影响的最窄层级：单个交接或两个已确认子项目之间的能力归属冲突只重开受影响的命名边界，其余子项目保持稳定，二级不能静默合并子项目；只有一个或多个实施边界的决定被推翻，或整个产品组合身份、所有权模型被否定，才重开完整一级拆分。
...
Level 1 采用三阶段生成。第一阶段从用户输入、默认 `prd/`、人工指定资料目录、当前 feature PRD、已接受意图账本和正式业务来源中提取 `business_context` 与内部能力原子：真实业务主语 `product_subject`、业务对象 `business_objects`、业务动作 `operations`、可观察结果 `outcomes`、端到端 `business_chains` 和证据缺口。能力原子是“产品对哪个业务对象执行什么动作或控制，并产生什么可观察结果”的最小有来源陈述；每个原子明确拥有一个触发或输入、一个业务状态、一个可独立验收结果和一个下游交接，所属业务链必须逐字段保持一致。用户明确给出的业务根节点和已确认结构优先；未确认的拆分建议必须同时通过业务边界质量门。若用户给的是技术分层或不能独立核验的拆分，模型不能悄悄改写，也不能直接定案，而应保留原建议、提供一份有来源的业务拆分候选并请用户判断。能力覆盖表、阶段名、检查清单和模型自检只用于内部编译，不进入导图、问题、推荐或面向用户的说明。

第二阶段先平铺全部原子和一对一业务链，再按业务责任把它们分配给直接子项目。固定关系是：“一个来源能力形成一个能力原子；一个能力原子只归属一个直接子项目；一个直接子项目可以拥有多个能力原子和业务链”。能力原子保证来源不丢、责任不重叠，但不是自动创建项目的指令。每个候选直接子项目必须有单一业务目标、可独立核验的整体结果、内聚的业务对象/规则/状态，以及与相邻项目之间少量且明确的业务输入、输出、命令或事件；项目引用的业务链集合必须等于其能力原子引用业务链的集合。拆分是必做分析，不是数量指标；不设固定子项目数量。低延迟、共享存储、顺序处理或部署便利只是交付风险，不能单独决定项目边界。事务一致性或双向业务交接必须先分类：若由法规、合同或多方法律义务强制，则保留为业务不变量；否则只记为交付风险。禁止按前端、后端、数据库、API、适配器、引擎、消息队列、代码仓库、部署单元或团队拆分。候选项目标题若只靠“中心、枢纽、平台、系统、模块、工作台、网络、接口、适配器、数据库、队列、界面、报表”等容器词或实现词成立，应继续检查它拥有的业务状态、独立结果和交接契约；前面加上领域词也不能补救缺失的业务边界。Overview 用真实产品组合、业务系统或受控业务闭环命名，每个一级入口对应一个候选直接子项目，而不是一个原子。

`000-*` 的 Level 1 分图只为确认直接子项目边界服务。分图可以摘要目标、拥有范围、整体结果、命名 handoff、来源和未决边界，但不能继续生成该项目内部的模块树、完整流程、页面、字段、接口或实现结构。多条能力原子与业务链保留在机器可读引用和来源追踪里，不必逐条变成用户看到的下级项目。更细的第三级内容由对应 `001+` 在自己的 Level 2 Outline 中根据完整资料重新生成。

第三阶段在写入 Discovery JSON 前执行语义质量门，逐项检查来源覆盖、候选子项目独立性、职责纯度、通过显式业务契约实现的松散耦合、完整业务生命周期、来源术语保真、产品主语、问题决策价值、实现词清理和内部推理不外泄。写出前还要对标题、摘要、节点、候选、推荐、问题和最终响应做一次可见文本清洗：删除阶段名、内部能力表、检查过程、自检、文件创建授权、feature 目录、SP 路由和 renderer 说明；把“引擎、中心、枢纽、平台、服务、系统、模块、工作台、网络、API、接口、适配器、数据库、队列、UI、界面、BI、报表”等泛化实现标签换回有来源的业务对象、动作、控制或结果，并再次执行语义质量门。对每条一级陈述和二级业务链执行跨领域替换测试：若只替换产品名就能原样搬到无关行业，说明它仍是套话，必须改成有来源的领域对象、动作、控制、结果或明确证据缺口。不能把这些实现组件当成业务能力，也不能把“当前理解、全局认知、项目概览、先确认方向、保持根节点稳定、形成业务闭环、确保系统稳定”等套话当成产品内容。第一次失败时重新提取或分组；再次失败时停留在 `explore`，把缺少的证据、结果、责任边界或交接点交给用户判断，而不是用方法论文字填满导图。

`frame` 除原有目标、角色和核心问题门槛外，至少需要一条由用户确认或正式文档支持的完整业务链。只有模型候选，或缺少对象、动作、结果、来源时，继续停留在 `explore`，把缺口绑定到对应业务分支的问题，不用方法论说明填满导图。
...
一级和二级使用 `discovery`，三级才使用现有 digest-bound `confirmation`。Discovery 以 XMind 风格思维导图呈现当前产品全局，不再把长篇 Outline 或平铺问题列表当作主入口。每轮至少包含三类图：一张全局总图、一张或多张候选子项目边界分图、一张全局约束图。总图展示真实产品及候选直接子项目并提供分图入口；Level 1 子项目分图只承载边界判断需要的业务目标、拥有范围、整体结果、命名交接、来源和未决边界，所有非项目节点必须明确作为分析/边界说明，不得被解释成更下级项目。Level 2 才在具体 `001+` 内承载业务场景、规则、异常和首切片业务链。政策、合规、审计、权限、安全等影响多个候选项目的业务规则单列到全局约束图，并明确关联受影响的业务节点。Constitution 仍在独立快照中只读展示，不混入全局业务约束；只影响单一候选项目的规则留在该分图。

每张图和每个节点都要有稳定 ID；只有真实未决边界才生成问题，问题、候选和用户输入必须绑定到具体项目边界或分析节点，用户点击节点后只处理该分支的判断。每个高价值问题提供 2-4 个候选、一个推荐及理由、“以上都不适用”和自由输入，并要求用户明确选择操作：`confirm_candidate`、`add`、`replace`、`exclude` 或 `context_note`。针对候选子项目的有效选项应是保持当前直接边界、与指定相邻项目合并并重述职责、进一步拆分明确职责、延期或排除，不能针对每个能力原子机械地生成一次保留/合并题。每个候选（包括推荐项）必须通过 `business_chain_refs` 和 `capability_atom_refs` 引用当前项目的完整归属集合；没有来源和业务链依据的推荐无效。一级和二级的关键产品判断必须让用户深度参与；三级只能整理已确认内容并检查产品结构完整性，不能从 Constitution 推断业务事实，也不能把尚未确认的一级、二级选择埋进自动生成的三级细节。

Discovery 只收集拆分意图。用户通过有效响应确认边界后，父级 PRD 记录 `Project Decomposition`，并为每个独立子项目生成一份子项目交接（`Subproject Handoff`），至少包含稳定 ID、单一业务目标、目标角色、拥有的能力和业务对象、范围/非目标、上游输入、下游输出、继承的全局业务门禁、来源和未决问题。handoff 是方向和责任合同，不是子项目详细需求全集。Level 1 不自动创建 feature 目录、不决定实施顺序，也不授权 `/sp.specify`。`000` 永远是纯 Portfolio/统筹根，只能保留 PRD、Outline、全局约束、子项目交接、跨项目依赖和影响记录；它不能拥有或消费 `spec.md`、Flow、UI、Bundle、Plan、Tasks、analysis、gate 或实现产物。任何统一壳、组合控制台或跨项目业务旅程，都必须先分配给一个 `001+` 的可验收实施子项目。已确认拆分的父级项目不能继续作为一个整体进入 `/sp.specify`；每个子项目应分别建立 feature 目录，读取自己的 handoff、默认 `prd/` 和人工指定资料，完成自己的 `/sp.prd` 和 Outline 确认。若用户确认保留为单项目，也不能把 `000` 变成实施 feature；应创建至少一个 `001+` 实施边界，或继续停留在 Outline 决策阶段。
...
导图必须主动控制并均匀分配信息密度：单图最多 18 个可见节点、最多 3 层、每个节点最多 4 个直接子节点；当单图达到 8 个节点时，任一层不得承载超过 60% 的节点。超出预算时应重排层级，或增加总览图、价值流分图和续图；不能通过减少候选数量、合并能力原子、缩字、堆叠或把大部分信息集中到同一层来勉强展示。页面主操作是“写入项目”，固定写入 pending 响应；`outline-discovery-response-*.json` 下载只作为写回失败后的显式降级。页面完成、localStorage、本地写回或下载动作都不能授权 `/sp.specify`，也不能把状态推进到 `AWAITING_OUTLINE_CONFIRMATION` 或 `READY_FOR_SPECIFY`。

Discovery 的生成数据、用户响应、意图账本和正式确认必须分离：`specs/<feature>/prd/review/outline-discovery-data.json` 是可刷新候选，`outline-discovery-response-pending.json` 是页面机械写入、等待下一次命令消费的非授权响应，`outline-discovery-response-*.json` 是失败降级文件，`specs/<feature>/prd/review/outline-intent-ledger.json` 是逻辑追加式决策账本，`specs/<feature>/prd/review/outline-review-data.json` 只属于三级正式确认。每次 `/sp.prd` 启动先检查 pending 响应，校验 feature、response、candidate、target、operation、schema version 和重复 delta，再追加账本；未知版本必须阻断，不能猜测、静默升级或降级成不兼容的旧合同。不能让 discovery 和 confirmation 数据包互相消费。从账本生成三级正式 review data 是 `/sp.prd` 的单向编译步骤：模型读取已接受账本状态以及当前 PRD/Outline，产生新的正式确认产物；不是 confirmation consumer 读取 discovery package。
