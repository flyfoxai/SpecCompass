...
Flow/UI 确认页采用统一模板：顶部显示简短 specCompass 机制说明和页面标题；左侧或中部展示带可见标签的 flow 图、流程表、UI 屏幕或项目 UI 全貌；右侧是一条约 280-320px 的窄确认栏，使用 Tiffany Blue `#0ABAB5` 作为主色，包含 batch 摘要、意见输入、逐项 approve/defer/reject 控件和批量确认按钮。确认页必须显示写回路径，并能让用户清楚知道确认结果需要进入 git 跟踪。
