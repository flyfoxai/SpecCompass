...
每个 `explore` 展开窗口采用三阶段生成。第一阶段从当前展开根的父级方向和继承约束出发，重新读取用户输入、默认 `prd/`、人工指定资料目录、当前 feature PRD、已接受意图账本和正式业务来源，提取本窗口的 `business_context` 与内部能力原子：真实业务主语 `product_subject`、业务对象 `business_objects`、业务动作 `operations`、可观察结果 `outcomes`、端到端 `business_chains` 和证据缺口。能力原子是“当前单元对哪个业务对象执行什么动作或控制，并产生什么可观察结果”的最小有来源陈述；每个原子明确拥有一个触发或输入、一个业务状态、一个可独立验收结果和一个下游交接，所属业务链必须逐字段保持一致。用户明确给出的节点和已确认父路径优先；未确认的拆分建议必须通过业务边界质量门。能力覆盖表、阶段名、检查清单和模型自检只用于内部编译，不进入导图、问题、推荐或面向用户的说明。

第二阶段先平铺当前根单元的全部原子和一对一业务链，再比较候选划分并分配给直接子单元。固定关系是：“一个来源能力形成一个能力原子；一个能力原子只归属一个直接子单元；能力原子负责来源覆盖，不负责决定项目数量”。独立状态、结果、角色、规则集或明确 handoff 是拆分信号；只有当拆开后能降低模型面对的业务与实现复杂度，并且子单元能通过稳定业务交接协作时，才形成独立边界。多个原子服务同一业务目标和生命周期或责任主体，且拆开会增加协调、重复状态或验收割裂时，可以由模型归入同一子单元。目标仍是功能单一、松散耦合，但不是默认一原子一项目，也不是默认合成少数大项目。

每个 Outline 单元必须有自己的业务目标、可核验整体结果、内聚的业务对象/规则/状态和来源；父单元不能因为已经拆出孩子而变成空白目录。单元引用的业务链集合必须等于其能力原子引用业务链的集合。多原子单元写出机器可读 `grouping_basis`，说明共同业务目标、共同生命周期或责任主体，以及拆开后具体会增加什么协调、重复状态或验收损失。耦合不变量的并集必须覆盖该单元全部能力原子，并把拆分反证中的全部备选责任组连成一个整体；稳定业务交接也必须连通拆分方案全部责任组。只证明其中两个原子有关，不能替整个大组提供内聚证据。文档明确给出的共同负责人或生命周期保留原始名称、精确锚点和原文摘录；模型根据业务事实补出的共同责任保持 `ai-proposed`，不能为方便归组改写成 `doc`。schema v6 允许本轮重新读取原始 PRD 后推导出的 `ai-proposed` 归组进入 Discovery 候选树，但必须声明 `proposal_origin: current-discovery`、提供完整 Web 保留/拆分方案，并让归组、耦合和交接依据只引用原始业务资料、人工指定资料或已确认父级方向；当前 feature PRD、memory、feature map 和旧 Outline 不能为这个提案自证。`unresolved` 仍不能支撑已经选定的非根归组。这样既避免旧候选自我复制，也避免机制退化成一原子一项目。归组不等于“已经不能继续拆”。是否继续拆由 `decomposition_basis` 判断，是否到达末端由 `terminal_basis` 判断。低延迟、共享存储、顺序处理、同一阶段、同一页面、部署便利、代码仓库或团队只是交付信息，不能单独决定项目边界；但同一业务数据责任若完整拥有多来源接收、标准化、质量与版本/血缘、留存和可信交付，可以形成一个内聚的数据边界，边界依据是数据业务生命周期而不是数据库产品。

schema v6 的每个 Outline 单元还必须携带完整的 `project_boundary` 项目边界合同，明确自有责任、范围、自有业务对象、非目标、上游输入、下游输出、独立验收和未决边界。自有业务对象必须与该单元能力原子实际使用的对象完全一致，不能夹带无关对象。上下游合同要覆盖该单元全部业务链，并由对应业务链的来源支持。`grouping_basis` 只证明多个能力原子为什么暂时放在一起，不能代替“这本身是一个什么项目”的定义；只写得出抽象共同目标，却写不出具体责任、排除项、独立验收和输入输出合同的归组，不能进入候选项目树。模型补出的完整边界可以作为 `ai-proposed` 候选进入 Web 审核，但在人工确认前不能成为基线。

拆分是必做分析，不是数量指标；不设固定子单元数量、偏好区间、一个原子一个项目的映射或整棵树的最大深度，也不要求越细越好。继续拆分能降低单元内部的状态、规则、角色、验收或实现复杂度，并能通过稳定业务 handoff 连接时，应继续拆；维持一个内聚单元的总复杂度更低时就保持在一起。拆分后只会增加协调、重复状态、人工交接、验收割裂或管理负担，或者业务目标已经不能独立验收时，才标记为 `terminal`。禁止按前端、后端、数据库、API、适配器、引擎、消息队列、代码仓库、部署单元或团队拆分。
