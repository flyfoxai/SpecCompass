...
当前口径：用户可见核心命令必须保持 `/sp.*`，例如 `/sp.specify`、`/sp.plan`、`/sp.analyze`。Codex 按 upstream skills 机制安装到 `.agents/skills/sp-<name>/SKILL.md`，并生成面向 `/prompt::sp.<name>` 的 prompt/plugin 入口；真实斜杠菜单是否显示必须以当前 Codex 客户端实测为准。Claude 安装到 `.claude/commands/sp.<name>.md`，并显示为 `/sp.<name>`。不要把 Codex 的 on-disk `sp-<name>` 技能包目录当成用户调用语法。extension / preset 命名空间继续使用 upstream 风格的 `speckit.<extension>.<command>`。
...
- Codex skills 物理目录：`.agents/skills/sp-plan/SKILL.md` 等；同时生成面向 `/prompt::sp.plan` 的 prompt/plugin 入口，真实 UI 是否显示需实测。
...
Codex 判断：原先“核心 skill 目录也必须使用 `sp.*`”属于历史误判，已废弃。当前修复方向是：Codex 保持 upstream-style `.agents/skills/sp-<name>/SKILL.md` 作为 executable skill package，同时生成 `.codex/prompts/sp.<name>.md` 和 plugin command 文件作为 `/prompt::sp.<name>` 目标入口；真实斜杠菜单是否显示需以当前 Codex 客户端实测为准。Claude 保持 `.claude/commands/sp.<name>.md`，并清理 `.claude/skills` 下的 core variants；旧 `.codex/commands`、旧 `.codex/skills`、以及 `.codex/prompts/sp-*.md` / `.codex/prompts/speckit*.md` 作为 stale residue 清理，但 `.codex/prompts/sp.*.md` 必须保留并由安装刷新。
