...
当前口径：用户可见核心命令必须保持 `/sp.*`，例如 `/sp.specify`、`/sp.plan`、`/sp.analyze`。Codex、Claude 等 skills 宿主的核心 skill 目录也使用 `sp.*`，例如 `sp.analyze/SKILL.md`，避免宿主把旧 `sp-*` 目录暴露成第二套命令。extension / preset 命名空间继续使用 upstream 风格的 `speckit.<extension>.<command>`。
...
- skills 物理目录：`.agents/skills/sp.plan/SKILL.md`、`.claude/skills/sp.plan/SKILL.md` 等。
- extension 命令：继续使用 `/speckit.<extension>.<command>`。

### 2. `SKILL.md` frontmatter `name`

更新结论：真实 ASK 安装检查已经证明 Codex 会把 `.agents/skills/sp-*` 目录直接暴露为可用 skill 名称，Claude 也会显示旧 `/speckit.*` 命令面。因此，核心 `SKILL.md` frontmatter `name` 和 skill 目录必须一起改为 `sp.*`。

Codex 判断：原先“`sp-*` 只是内部实现细节”的判断不成立。当前修复方向是让核心 skill 目录、frontmatter `name`、 companion command 文件和用户调用形式全部收敛到 `sp.*`；`sp-*` 仅作为 legacy residue 被清理或兼容迁移。
...
- 运行时默认 catalog URL 改为 `flyfoxai/openSpecs`。
...
- 旧核心 dotted skill 目录，例如 `speckit.plan`，迁移到 `sp.plan`。
...
- 运行时 catalog 示例链接从 upstream `github/spec-kit` 改为当前 fork `flyfoxai/openSpecs`，避免二次开发者照示例接回 upstream。
...
背景：`sp-*` 目录不是纯内部细节，宿主可能直接显示它。继续保留会造成用户看见 `/sp-analyze` 或 `sp-analyze`，偏离 `/sp.analyze` 设计目标。

建议：核心 skill 目录和 frontmatter name 使用 `sp.*`；安装 refresh 清理旧 `sp-*`、`speckit-*`、`speckit.*` 核心命令面。Forge 由于宿主要求 hyphenated frontmatter，保留 integration-specific `sp-*` name，但其命令文件仍是 `sp.*.md`。
