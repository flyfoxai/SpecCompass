...
当前口径：用户可见核心命令必须保持 `/sp.*`，例如 `/sp.specify`、`/sp.plan`、`/sp.analyze`。`sp-*` 只允许作为 skills 宿主的内部目录或物理文件名。extension / preset 命名空间继续使用 upstream 风格的 `speckit.<extension>.<command>`。
...
Gemini 追问复核意见：暂不把 `name: sp-analyze` 改成 `name: sp.analyze`。理由是当前结构更接近 upstream skills 机制，且还没有真实宿主证据证明 `name: sp-*` 会破坏 `/sp.*` 用户入口。

Codex 判断：同意。当前优先保证用户文档、命令正文、next-step 输出统一为 `/sp.*`。如果未来真实 Codex / Claude 宿主证明 frontmatter `name` 会强制展示 `/sp-*`，再做宿主专用映射。
...
- 测试已更新，避免 `speckit-plan` 与 `sp-plan` 并存。
...
背景：当前 `SKILL.md` frontmatter 仍是 `name: sp-analyze` 这类 hyphenated 名称。Gemini 建议暂不改，Codex 同意。

影响：如果某个宿主强依赖 `name` 字段作为用户命令展示，可能仍显示 `/sp-analyze`。但目前项目规则和命令正文已经要求用户入口使用 `/sp.analyze`。

建议：保留现状；发布前继续做真实 Codex / Claude 宿主 smoke test。如果真实证据证明宿主展示错误，再做 integration-specific frontmatter name 映射。
