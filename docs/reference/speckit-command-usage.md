...
能力原子用于来源覆盖，不是项目数量配额，也不要求一原子一项目。独立状态、结果、角色、规则或 handoff 是拆分信号；模型要比较拆开后的复杂度下降与新增协调成本，再选择功能单一、松散耦合且总复杂度更低的子单元边界。多原子单元必须用 `grouping_basis` 说明共同目标、共同生命周期/责任主体，以及拆开会增加的协调、重复状态或验收损失。PRD 没有直接规定项目架构时，模型可以用 `ai-proposed` 提出有理由的归组并交给 Web Discovery 确认；`unresolved` 不能支撑已经归组的子单元。每个父单元还要说明 `decomposition_basis` 或 `terminal_basis`，直接子单元必须不重叠且完整覆盖父单元；不能因共享页面、数据库、运行时、阶段或团队而合并，也不能为了数量或“足够细”而强拆。
