...
能力原子用于来源覆盖，不是项目数量配额，也不要求一原子一项目。独立状态、结果、角色、规则或 handoff 是拆分信号；模型要比较拆开后的复杂度下降与新增协调成本，再选择功能单一、松散耦合且总复杂度更低的子单元边界。多原子单元必须用 `grouping_basis` 说明共同目标、共同生命周期/责任主体，以及拆开会增加的协调、重复状态或验收损失。PRD 没有直接规定项目架构时，模型可以用 `ai-proposed` 提出有理由的归组并交给 Web Discovery 确认；本轮提案要声明 `proposal_origin: current-discovery`，只能引用原始业务资料、人工资料或确认父级依据，不能复用旧 feature PRD、memory 或 Outline 自证。`unresolved` 不能支撑已经归组的子单元。每个父单元还要说明 `decomposition_basis` 或 `terminal_basis`，直接子单元必须不重叠且完整覆盖父单元；不能因共享页面、数据库、运行时、阶段或团队而合并，也不能为了数量或“足够细”而强拆。完整拥有接收、标准化、质量/版本/血缘、留存和可信交付的同一业务数据生命周期，可以形成一个数据责任边界。

schema v6 的每个 Outline 单元都必须提供 `project_boundary` 项目边界合同，写清自有责任、范围、业务对象、非目标、上游输入、下游输出、独立验收和未决边界。自有业务对象必须与该单元能力原子实际使用的对象完全一致，不能为了把项目写大而夹带无关对象。上下游合同分别覆盖该单元全部业务链，并引用支持对应业务链的来源。`grouping_basis` 只是归组证据，不是项目定义；无法具体写出这份合同的抽象能力分组不能作为候选子项目。模型补出的完整边界仍保持 `ai-proposed`，通过 Web 审核后才能进入确认基线。
