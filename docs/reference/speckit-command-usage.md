...
能力原子用于来源覆盖，不是项目数量配额，也不要求一原子一项目。首次完整提取并通过校验后，`specs/<feature>/prd/source-capability-baseline.json` 只保存来源指纹和 Stage A 事实；来源、当前人工修正和已接受决定未变化时必须原样复用，旧项目分区不会进入该文件。Stage B 把相互冲突且有文档或人工权威的 owner/lifecycle 当成不可跨越的合并约束；共享同一权威身份是保留在一起的重要证据，但不机械禁止内部拆分。baseline 中相同的 `ai-proposed` 身份只是软线索，不能成为默认分区。模型草拟真正不同的 Web 方案，再以“项目内部复杂度 + 项目间协调成本”的总复杂度比较，每套方案给出内部复杂度、协调成本和名次，实际生成项必须排第 1，但仍保持待审核提案；真正并列时允许多个第 1，选中项只是页面临时推荐，全部并列方案交给人工决定。单原子或没有第二套有效方案时不得制造假分区。技术层、权威跨界、运行时拓扑、通用容器、`责任单元/责任域` 等内部分类、填充节点和目标数量都是硬拒绝项。多原子子单元只在全部原子真实共享时填写 owner/lifecycle，并需要一个覆盖全组的自然语言业务不变量和一个 separation test：完整给出最佳拆分备选、真实跨组事实、确有的重复状态，以及保留当前归组总复杂度更低的理由。同级 frontier 还必须分别给出本项目自己的下一层业务拆分，不能复制通用说明。模型提案保持 `ai-proposed` 和 `proposal_origin: current-discovery`，只引用原始业务资料、人工资料或确认父级依据，并进入 Web 保留/拆分审核。每个父单元还要说明 `decomposition_basis` 或 `terminal_basis`，每个子项目保留完整 `project_boundary`；完整拥有接收、标准化、质量/版本/血缘、留存和可信交付的数据业务生命周期仍可以形成数据责任边界。
...
schema v6 的每个 Outline 单元都必须提供 `project_boundary` 项目边界合同，写清自有责任、范围、业务对象、非目标、上游输入、下游输出、独立验收和未决边界。每个单元还必须提供 `project_title`，由自有业务对象与 `overall_outcome` 派生为具体实体名；它不能是 owner/lifecycle slug、“X责任”“X生命周期”或“责任单元/责任域”等内部分类词。它必须复用至少一个自有对象的具体名词，命名该单元拥有并治理的记录或聚合（回答“它拥有什么”，而非“它做什么”）；不得是发布、授权、决定、裁决、披露、评测、审计、恢复、执行、采集、处理、管理、计算、路由、监控等动作或治理词（除非该词本身就是来源命名的自有对象），也不得用事实、证据、信息、数据、版本、特征、结果等伞词替换对象的具体名词。`unresolved_boundary` 必须描述本单元自己的边界张力，同级兄弟不得复用同一句模板。自有业务对象必须与该单元能力原子实际使用的对象完全一致，不能为了把项目写大而夹带无关对象。上下游合同分别覆盖该单元全部业务链，并引用支持对应业务链的来源。`grouping_basis` 只是归组证据，不是项目定义；无法具体写出这份合同的抽象能力分组不能作为候选子项目。模型补出的完整边界仍保持 `ai-proposed`，通过 Web 审核后才能进入确认基线。
