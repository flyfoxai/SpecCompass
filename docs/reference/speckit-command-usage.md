...
- SP 的目标是宿主入口一致：slash-command 宿主使用 `/sp.*`；Codex 使用 skills，输入 `$` 或运行 `/skills` 后选择 `sp-*`。
...
- SP 可以额外生成 `.codex/prompts/sp.*.md`、`.agents/plugins/marketplace.json` 和 `plugins/sp/` 作为兼容产物。
- 不能只因为 prompt/plugin 文件存在，就在 README 或使用文档里承诺 Codex UI 一定显示 `/sp.*` 或 `/prompt::sp.*`。
- Codex 下应指导用户输入 `$` 或运行 `/skills`，选择对应 `sp-*` skill。
...
- 因此，SpecCompass 对 Codex 的主入口应是 `$` 或 `/skills` 中的 `sp-*` skill。
...
- SP 额外生成 `.codex/prompts/sp.*.md`、`.agents/plugins/marketplace.json` 和 `plugins/sp/`，作为 prompt/plugin 兼容产物。
- 本地曾能看到 `codex plugin list` 中存在 SP plugin 且处于 installed/enabled 状态，也能看到 plugin cache 中有 `commands/sp.*.md`。
- 但上述结果只证明文件和 plugin 注册存在，不能等同于 Codex UI slash autocomplete 一定显示 `/sp.*` 或 `/prompt::sp.*`。
...
- “斜杠菜单显示 `/sp.*` 或 `/prompt::sp.*`”不是当前 Codex 安装成功标准，不能用它否定 skills 安装成功。

后续修复方向：

- 不应再把 Codex slash 显示问题简单归因于“没有对齐原版 Spec Kit”，因为原版当前也以 skills 为主。
- README 和安装文档必须把 Codex 主入口写成 `$` 或 `/skills` 中的 `sp-*` skill。
- 如果未来 Codex 官方重新提供稳定 prompt/plugin 调用接口，应先作为兼容增强验证，不应替代 skills 主路径。

推荐产品策略：

- 主路径：继续保证 `.agents/skills/sp-*/SKILL.md` 正确生成，因为这是当前最接近 upstream 的 Codex 支持方式。
- 增强路径：保留 `.codex/prompts/` 和 Codex plugin marketplace 文件作为兼容产物。
- 兜底路径：安装后提供检查命令或文档步骤，明确区分“已生成 skills”“已注册 plugin”“slash UI 是否显示”。如果 slash UI 不显示，不判定安装失败。
...
- Codex 使用 `$` 或 `/skills` 能选择 `sp-*` skill；不要把 `/sp.*` 或 `/prompt::sp.*` slash menu 可见性作为成功标准。
