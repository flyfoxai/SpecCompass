...
21. 任意 `explore` 窗口都先完整提取能力原子，再根据单一业务责任、复杂度下降和交接成本选择子单元边界；既不把原子数量机械当作项目数量，也不允许用宽泛主题把互相独立的责任塞进少数大桶。多原子单元必须提供完整 `grouping_basis`；Schema、CLI 和浏览器接受 `ai-proposed` 的有理由归组作为待确认 Discovery 候选，但拒绝用 `unresolved` 归组生成既成边界，并继续拒绝遗漏、重复归属、chain 集合错配和跨同级单元状态/结果重叠。
