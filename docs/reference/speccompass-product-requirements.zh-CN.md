...
- `000-*` 是唯一顶级 Outline 单元，但不是空壳协调器。它保留自己的产品目标、整体结果、来源和全局约束，同时可以递归拆出多个直接子单元。具有独立状态、独立结果或明确业务 handoff 的能力默认保持为松散耦合、单一职责的候选单元；多个能力只有在正式 PRD 或已确认人工决定给出充分的共同目标、生命周期/责任主体和拆开后的验收损失时才可聚合。模型发现可能的合并方式但没有这类权威依据时，当前树仍按独立子单元生成，只能把具名合并方案放进 Web Discovery 供人工决定；不能先聚合成大项目再要求人工接受。能力原子是来源覆盖单位，不是机械项目数量。
...
21. 任意 `explore` 窗口都默认把具有独立状态、结果或 handoff 的能力保留为松散耦合的单一职责子单元；只有正式 PRD 或已确认人工决定提供完整 `grouping_basis` 时，才允许把多个能力原子和业务链唯一分配给同一个非根单元。CLI、Schema 和浏览器校验都拒绝以 `ai-proposed` 或 `unresolved` 权威生成这种多原子单元；未经确认的合并只能作为 Web Discovery 选项，不能改变当前生成树。仍拒绝遗漏、重复 owner、chain 集合错配和跨同级单元状态/结果重叠，也不会把原子数量机械当作项目数量指标。
