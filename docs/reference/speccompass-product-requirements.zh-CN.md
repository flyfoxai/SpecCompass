...
21. 任意 `explore` 窗口都先完整提取能力原子，再根据单一业务责任、复杂度下降和交接成本选择子单元边界；既不把原子数量机械当作项目数量，也不允许用宽泛主题把互相独立的责任塞进少数大桶。多原子单元必须提供完整 `grouping_basis`：有来源的耦合证据覆盖全部被归组原子并连通全部备选责任组，稳定交接覆盖拆分方案中的全部责任组，文档权威带可核对原文，同级复杂度比较不能复用套话。PRD 没有明确命名共同负责人或生命周期时，模型补全只能标记为 `ai-proposed` 并进入 Web 拆分/保留决策，不能冒充 `doc`。v6 中未确认的 `ai-proposed` 或 `unresolved` 归组、共同负责人或生命周期只能作为 Web 候选，不能进入非根生成树；此时按来源支持的独立原子生成窗口。Schema、CLI 和浏览器拒绝把旧 feature PRD 的 AI 候选列表重新当成分区证据，也拒绝局部证据支撑整个大组、遗漏、重复归属、chain 集合错配和跨同级单元状态/结果重叠。
