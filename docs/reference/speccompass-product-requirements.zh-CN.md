...
- `000-*` 是唯一顶级 Outline 单元，但不是空壳协调器。它保留自己的产品目标、整体结果、来源和全局约束，同时可以递归拆出多个直接子单元。能力原子只负责证明来源覆盖，不等于项目数量；独立状态、结果、角色、规则或 handoff 是拆分信号，不是“一项能力必须成为一个项目”的硬规则。模型必须比较候选划分：拆开后能降低状态、规则、验收和实现复杂度，且能用稳定业务交接连接时拆开；多个能力服务同一业务目标和生命周期，拆开反而增加协调、重复状态或验收割裂时可以归为一个子单元。模型可以补充 PRD 未明确给出的项目架构，但要写出 `grouping_basis` 并作为 Discovery 候选由人工确认；本轮根据原始 PRD 重新推导的模型归组可以进入候选树，不能因为尚未确认就退化成“一原子一项目”。重大边界选择必须提供方案、影响和推荐。整个过程不设固定项目数量，也不追求越细越好。
...
- 每个 Outline 单元都必须保留自己的业务目标、整体结果、能力/业务链引用和来源；拥有子单元不等于父单元没有内容。已展开单元要说明拆分如何降低复杂度，待继续单元不能提前声称已经终止，终端单元要说明继续拆分为何会增加管理、交接或实现复杂度。直接子单元必须不重叠且完整覆盖父单元的能力和业务链。
...
21. 任意 `explore` 窗口都先完整提取能力原子，再根据单一业务责任、复杂度下降和交接成本选择子单元边界；既不把原子数量机械当作项目数量，也不允许用宽泛主题把互相独立的责任塞进少数大桶。多原子单元必须提供完整 `grouping_basis`：有来源的耦合证据覆盖全部被归组原子并连通全部备选责任组，稳定交接覆盖拆分方案中的全部责任组，文档权威带可核对原文，同级复杂度比较不能复用套话。PRD 没有明确命名共同负责人或生命周期时，模型补全只能标记为 `ai-proposed` 并进入 Web 拆分/保留决策，不能冒充 `doc`。v6 允许本轮根据新提取原子形成的 `ai-proposed` 归组进入 Discovery 候选树，但必须声明 `proposal_origin: current-discovery`，归组、耦合和交接依据只能来自原始业务资料、人工指定资料或已确认父级引用，不能来自当前 feature PRD、memory、feature map 或旧 Outline；`unresolved` 仍不能支撑已经选定的非根归组。共同使用某个数据库不构成合并理由；若同一业务数据责任完整拥有多来源接收、标准化、质量与版本/血缘、留存和可信交付，则可以作为一个数据边界。Schema、CLI 和浏览器拒绝把旧 feature PRD 的 AI 候选列表重新当成分区证据，也拒绝局部证据支撑整个大组、遗漏、重复归属、chain 集合错配和跨同级单元状态/结果重叠。
