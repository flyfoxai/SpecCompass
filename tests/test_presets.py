...
class TestPresetManifest:
    ...
    def test_non_core_sp_dot_command_namespace_rejected(self, temp_dir, valid_pack_data):
        """Presets may not create new commands under the reserved sp.* namespace."""
        valid_pack_data["provides"]["templates"] = [
            {"type": "command", "name": "sp.research", "file": "commands/sp.research.md"}
        ]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
        with pytest.raises(PresetValidationError, match="reserved for core commands"):
            PresetManifest(manifest_path)

    def test_non_core_sp_hyphen_command_namespace_rejected(self, temp_dir, valid_pack_data):
        """Hyphenated sp-* command names are also reserved for core command aliases."""
        valid_pack_data["provides"]["templates"] = [
            {"type": "command", "name": "sp-research", "file": "commands/sp-research.md"}
        ]
        manifest_path = temp_dir / "preset.yml"
    ...
    def test_core_sp_command_override_allowed(self, temp_dir, valid_pack_data):
        """Presets may still override existing core commands with canonical sp.* names."""
        valid_pack_data["provides"]["templates"] = [
            {"type": "command", "name": "sp.plan", "file": "commands/sp.plan.md"}
        ]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
        manifest = PresetManifest(manifest_path)
        assert manifest.templates[0]["name"] == "sp.plan"

    def test_legacy_core_speckit_command_override_allowed(self, temp_dir, valid_pack_data):
        """Legacy speckit.<core> preset overrides remain accepted for migration."""
        valid_pack_data["provides"]["templates"] = [
            {"type": "command", "name": "speckit.plan", "file": "commands/speckit.plan.md"}
        ]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
        manifest = PresetManifest(manifest_path)
        assert manifest.templates[0]["name"] == "speckit.plan"
...
class TestLeanPreset:
    ...
    def test_lean_manifest_valid(self):
        ...
        assert manifest.version == "1.0.0"
        assert len(manifest.templates) == 8  # 8 commands
    ...
    def test_lean_commands_have_no_scripts(self):
        ...
        for name in LEAN_COMMAND_NAMES:
            cmd_path = LEAN_PRESET_DIR / "commands" / f"{command_filename_base(name)}.md"
            content = cmd_path.read_text()
