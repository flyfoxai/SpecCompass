...
class TestCommandStep:
    ...
    def test_execute_basic(self):
        ...
        assert result.output["command"] == "sp.specify"
...
class TestIfThenStep:
    ...
    def test_execute_then_branch(self):
        ...
        config = {
            ...
            "then": [{"id": "a", "command": "sp.tasks"}],
            "else": [{"id": "b", "command": "sp.plan"}],
        }
        result = step.execute(config, ctx)
...
class TestSwitchStep:
    ...
    def test_execute_matches_case(self):
        ...
        config = {
            ...
            "cases": {
                "approve": [{"id": "plan", "command": "sp.plan"}],
...
class TestWhileStep:
    ...
    def test_execute_condition_true(self):
        ...
        config = {
            ...
            "max_iterations": 5,
            "steps": [{"id": "fix", "command": "sp.implement"}],
        }
        result = step.execute(config, ctx)
        assert result.output["condition_result"] is True
        assert len(result.next_steps) == 1
...
class TestDoWhileStep:
    ...
    def test_execute_always_runs_once(self):
        ...
        config = {
            ...
            "condition": "{{ false }}",
            "max_iterations": 3,
            "steps": [{"id": "refine", "command": "sp.specify"}],
        }
        result = step.execute(config, ctx)
        assert len(result.next_steps) == 1
...
class TestFanOutStep:
    ...
    def test_execute_with_items(self):
        ...
        config = {
            ...
            "max_concurrency": 3,
            "step": {"id": "impl", "command": "sp.implement"},
        }
        result = step.execute(config, ctx)
...
class TestWorkflowValidation:
    ...
    def test_unknown_workflow_fields_warn_without_blocking(self):
        from specify_cli.workflows.engine import (
            WorkflowDefinition,
            validate_workflow,
            validate_workflow_warnings,
        )
...
steps:
  - id: step-one
    command: sp.specify
    x-retry-policy: conservative
""")

        assert validate_workflow(definition) == []
        warnings = validate_workflow_warnings(definition)
        assert any("custom_top_level" in warning for warning in warnings)
        assert any("x-owner" in warning for warning in warnings)
        assert any("x-ui-hint" in warning for warning in warnings)
        assert any("x-retry-policy" in warning for warning in warnings)
...
"""
        ...
        assert state.step_results["step-one"]["output"]["command"] == "sp.specify"
...
steps:
  - id: specify
    type: shell
    run: "echo sp.specify {{ inputs.feature }}"
