...
class TestKimiLegacyMigration:
    """Test Kimi legacy names → canonical hyphenated skill directory migration."""

    def test_migrate_legacy_core_dotted_to_sp_skill(self, tmp_path):
...
class TestKimiNextSteps:
    ...
    def test_next_steps_show_skill_invocation(self, tmp_path):
        """Kimi next-steps guidance should display /sp.* usage."""
        import os
        ...
        assert "/skill:sp-constitution" not in result.output
        assert "/speckit.constitution" not in result.output
        assert "Optional commands that you can use for your specs" in result.output
