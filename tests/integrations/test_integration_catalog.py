...
VALID_DESCRIPTOR = {
    ...
    "provides": {
        "commands": [
            {"name": "sp.specify", "file": "templates/sp.specify.md"},
        ],
        "scripts": [],
    },
}
...
class TestIntegrationDescriptor:
    ...
    def test_valid_descriptor(self, tmp_path):
        ...
        assert desc.commands[0]["name"] == "sp.specify"
        assert desc.scripts == []

    def test_legacy_core_command_descriptor_still_allowed(self, tmp_path):
        data = json.loads(json.dumps(VALID_DESCRIPTOR))
        data["provides"]["commands"][0] = {
            "name": "speckit.specify",
            "file": "templates/speckit.specify.md",
        }
        p = self._write(tmp_path, data)
        desc = IntegrationDescriptor(p)
        assert desc.commands[0]["name"] == "speckit.specify"
