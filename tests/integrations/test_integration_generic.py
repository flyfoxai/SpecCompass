...
class TestGenericIntegration:
    ...
    def test_setup_creates_md_files(self, tmp_path):
        ...
        for f in cmd_files:
            assert f.name.startswith(f"{command_filename_base('plan').split('.', 1)[0]}.")
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest("generic", tmp_path)
        i.setup(tmp_path, m, parsed_options={"commands_dir": ".custom/cmds"})
        plan_file = tmp_path / ".custom" / "cmds" / f"{command_filename_base('plan')}.md"
    ...
    def test_complete_file_inventory_sh(self, tmp_path):
        ...
        expected = sorted(
            [f".myagent/commands/{command_filename_base(stem)}.md" for stem in command_stems()]
            + shared_init_files(
