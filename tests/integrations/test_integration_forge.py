...
class TestForgeIntegration:
    ...
    def test_command_filename_md(self):
        forge = get_integration("forge")
        assert forge.command_filename("plan") == f"{command_filename_base('plan')}.md"
    ...
    def test_directory_structure(self, tmp_path):
        ...
        command_files = sorted(commands_dir.glob(f"{command_filename_base('plan').split('.', 1)[0]}.*.md"))
        assert len(command_files) == len(expected_commands)
        prefix = f"{command_filename_base('plan').split('.', 1)[0]}."
        actual_commands = {f.name.removeprefix(prefix).removesuffix(".md") for f in command_files}
    ...
    def test_templates_are_processed(self, tmp_path):
        ...
        forge.setup(tmp_path, m)
        commands_dir = tmp_path / ".forge" / "commands"
        for cmd_file in commands_dir.glob(f"{command_filename_base('plan').split('.', 1)[0]}.*.md"):
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest("forge", tmp_path)
        forge.setup(tmp_path, m)
        plan_file = tmp_path / ".forge" / "commands" / f"{command_filename_base('plan')}.md"
    ...
    def test_uses_parameters_placeholder(self, tmp_path):
        ...
        # Check all generated core command files
        ...
        checklist = commands_dir / f"{command_filename_base('checklist')}.md"
        if checklist.exists():
    ...
    def test_name_field_uses_hyphenated_format(self, tmp_path):
        """Verify that injected name fields use hyphenated format (sp-plan, not sp.plan)."""
        from specify_cli.integrations.forge import ForgeIntegration
        from specify_cli.agents import CommandRegistrar
        forge = ForgeIntegration()
        m = IntegrationManifest("forge", tmp_path)
        ...
        for cmd_file in commands_dir.glob("sp.*.md"):
            ...
            assert name_value.startswith("sp-"), (
                f"{cmd_file.name} name field should start with 'sp-': {name_value}"
            )
