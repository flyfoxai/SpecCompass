...
"""
    ...
    def test_rewrite_project_relative_paths_keeps_feature_memory_paths(self):
        """Feature memory paths must not be rewritten to project memory."""
        from specify_cli.agents import CommandRegistrar as AgentCommandRegistrar

        body = (
            "Read memory/open-items.md and memory/trace-index.md.\n"
            "Also read memory/constitution.md when project rules are needed.\n"
        )

        rewritten = AgentCommandRegistrar.rewrite_project_relative_paths(body)

        assert "memory/open-items.md" in rewritten
        assert "memory/trace-index.md" in rewritten
        assert ".specify/memory/open-items.md" not in rewritten
        assert ".specify/memory/trace-index.md" not in rewritten
        assert ".specify/memory/constitution.md" in rewritten

    def test_project_memory_rewrite_allowlist_matches_template_memory_files(self):
        """Project-level memory rewrite names should stay aligned with shipped templates."""
        from specify_cli.agents import CommandRegistrar as AgentCommandRegistrar

        memory_dir = Path(__file__).resolve().parent.parent / "templates" / "project" / ".specify" / "memory"
        project_memory_files = sorted(path.stem for path in memory_dir.glob("*.md"))
        body = "\n".join(f"Read memory/{name}.md." for name in project_memory_files)

        rewritten = AgentCommandRegistrar.rewrite_project_relative_paths(body)

        for name in project_memory_files:
            assert f".specify/memory/{name}.md" in rewritten
            assert f"Read memory/{name}.md." not in rewritten
    ...
    def test_unregister_commands_for_codex_skills_uses_mapped_names(self, project_dir):
        ...
        core_skill = skill_directory_name("sp.specify")
        (skills_dir / core_skill).mkdir(parents=True)
        (skills_dir / core_skill / "SKILL.md").write_text("body")
        ...
        assert not (skills_dir / core_skill / "SKILL.md").exists()
...
class TestExtensionCatalog:
    ...
    def test_default_catalog_urls_use_open_specs_fork(self, temp_dir):
...
class TestHookInvocationRendering:
    ...
    def test_kimi_hooks_render_canonical_invocation(self, project_dir):
        """Kimi projects should render canonical /sp.* for core hook commands."""
        init_options = project_dir / ".specify" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_codex_hooks_render_canonical_invocation(self, project_dir):
        """Codex projects with --ai-skills should render canonical /sp.* for core commands."""
        init_options = project_dir / ".specify" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert execution["command"] == "sp.tasks"
        assert execution["invocation"] == "/sp.tasks"
    ...
    def test_extension_command_uses_canonical_invocation(self, project_dir):
        """Multi-segment extension command ids should keep canonical dotted display."""
        init_options = project_dir / ".specify" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        init_options.write_text(json.dumps({"ai": "kimi", "ai_skills": False}))
        ...
        assert "Executing: `/speckit.test-ext.hello`" in message
        assert "EXECUTE_COMMAND: speckit.test-ext.hello" in message
        assert "EXECUTE_COMMAND_INVOCATION: /speckit.test-ext.hello" in message
    ...
    def test_hook_executor_caches_init_options_lookup(self, project_dir, monkeypatch):
        ...
        hook_executor = HookExecutor(project_dir)
        assert hook_executor._render_hook_invocation("sp.plan") == "/sp.plan"
        assert hook_executor._render_hook_invocation("sp.tasks") == "/sp.tasks"
