...
CHECK_PREREQUISITES_SH = PROJECT_ROOT / "scripts" / "bash" / "check-prerequisites.sh"
...
def test_seeded_feature_scaffold_is_not_reported_as_tasks_prepared(tmp_path: Path):
    ...
    (feature_dir / "clarifications.md").write_text(
        "# Clarifications\n<!-- SP_STAGE_SEED: clarifications -->\n",
        encoding="utf-8",
    )
    (feature_dir / "gate.md").write_text(
        "# Gate\n- Current Verdict: `Pending Review`\n",
        encoding="utf-8",
    )
...
def test_feature_primary_workset_prefers_main_journey_over_alphabetical_order(tmp_path: Path):
    ...
    worksets_dir.mkdir(parents=True)
...
def test_bash_check_prerequisites_reports_no_active_feature_as_state(tmp_path: Path):
    ...
    scripts_dir.mkdir(parents=True)
    shutil.copy(COMMON_SH, scripts_dir / "common.sh")
    shutil.copy(CHECK_PREREQUISITES_SH, scripts_dir / "check-prerequisites.sh")
    ...
    assert result.returncode == 0, result.stderr
    assert "Feature directory not found" not in result.stderr
...
def test_powershell_seeded_feature_scaffold_is_not_reported_as_tasks_prepared(tmp_path: Path):
    ...
    command = f'. "{PROJECT_ROOT / "scripts" / "powershell" / "common.ps1"}"; Get-FeatureStageLabel -FeatureDir "{feature_dir}"'
...
@pytest.mark.skipif(not HAS_PWSH, reason="pwsh not available")
def test_powershell_feature_primary_workset_prefers_main_journey(tmp_path: Path):
    """PowerShell primary workset routing should mirror Bash."""
    worksets_dir = tmp_path / "specs" / "001-demo" / "memory" / "worksets"
    worksets_dir.mkdir(parents=True)
    (worksets_dir / "ws-decision-and-approval.md").write_text("# Decision\n", encoding="utf-8")
    (worksets_dir / "ws-primary-journey.md").write_text("# Primary\n", encoding="utf-8")

    command = f'. "{PROJECT_ROOT / "scripts" / "powershell" / "common.ps1"}"; Get-FeaturePrimaryWorkset -FeatureDir "{tmp_path / "specs" / "001-demo"}"'
    result = subprocess.run(["pwsh", "-NoProfile", "-Command", command], capture_output=True, text=True)
...
@pytest.mark.skipif(not HAS_PWSH, reason="pwsh not available")
def test_powershell_check_prerequisites_reports_no_active_feature_as_state(tmp_path: Path):
    """PowerShell prerequisite checks should mirror Bash fresh-project routing."""
