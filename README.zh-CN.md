...
- `/sp.flow` 先建立业务/架构流程基线；当流程是首次生成、高风险、大范围变化，或包含需要人工判断的关键点时，会进入确认过程。命令写入 `specs/<feature>/flows/review/flow-review-data.json`，并通过 SpecCompass Web 审核页展示，让审核人直接查看直观流程图、模块简介、流程说明和节点级确认点，而不是只读原始文件。
- `/sp.flow` 的审核页会为需要判断的节点生成说人话的决策卡片，包括推荐选项、备选项、各选项后果和推荐理由。审核人在右侧反馈/确认栏中选择“使用推荐”或提交其他选择及意见。浏览器点击只算本地草稿；只有确认摘要写入 `flow-confirmation.md` 后，后续命令才能把该 flow 当作已授权输入。
- `/sp.ui` 在相关 flow 已确认后运行，把流程承载的界面需求转成可审核的 UI 预览，包括页面、区域、组件、状态和可见交互决策。需要 UI 审核时，命令写入独立的 `specs/<feature>/ui/review/ui-review-data.json`；UI review data 不是 flow data，不能复用 flow 节点来冒充界面元素。
- `/sp.ui` 的审核页使用同一套 SpecCompass Web 审核界面，并按 Huashu 设计规则展示界面预览和右侧确认栏。模型会为布局、关键控件、状态、异常处理等设计判断点提供推荐选项和备选项。UI 选择只有写入 `ui-confirmation.md` 后，才能成为稳定的实现输入。
