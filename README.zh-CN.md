...
- **每个审核点都交代业务背景**：Flow 判断点说明触发时机、责任角色、结果和下一责任；每个 UI 界面说明为什么存在、谁在什么场景进入、要完成什么，以及设计依据来自哪些 Flow 事实
...
`/sp.flow` 先生成或刷新业务流程、架构流程和相关流程产物，再生成流程审核页，用来查看模块、流程图、判断点和推荐选项。每个判断点都要交代真实触发场景、责任角色、业务结果和下一责任，审核人才能看懂它在整体流程中的作用。

`/sp.ui` 先生成或刷新页面地图、界面结构、区域、控件、状态和关键交互选择，再生成界面审核页，用来进行可视化确认。每个 Screen 还要记录业务背景、主要使用角色、进入场景、用户目标、完成结果和 Flow 依据。Flow 只提供 UI 设计所需的业务事实，不能直接替代界面中的区域和控件。
