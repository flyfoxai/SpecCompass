...
- **提供推荐选择**：模型给出选项、影响和推荐理由，避免让审核人从空白处判断
...
3. 用 `/sp.flow` 审核业务流程和架构流程
4. 用 `/sp.ui` 审核界面结构和交互选择
...
`/sp.flow` 会生成流程审核页，用来查看模块、流程图、判断点和推荐选项。

`/sp.ui` 会生成界面审核页，用来查看页面预览、区域、控件、状态和关键交互选择。

两个审核页都使用右侧确认栏。审核人可以接受推荐，也可以选择其他方案并补充意见。浏览器页面用于辅助审阅，真正授权后续实现的是生成的确认文档。
