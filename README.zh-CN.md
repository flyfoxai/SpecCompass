...
主方法论见 [SP 项目的方法论](./docs/reference/sp-project-methodology.md)。从 0 到 1 的可选产品发现、PRD 处理、阻塞闭环、代码续作和多 agent 规则都已合并到主方法论中。简要来说：
...
- `/sp.flow` 使用结构化流程文件和可渲染流程图提前核对方向。首次生成、高风险或大批量流程变化，在用户按可见标签确认或选择修复方案前都保持草稿。
- `/sp.ui` 在 flow 之后运行：先收集每个界面需要承载的流程元素，再把它们拼成合理界面。
- `/sp.ui` 使用结构化 UI 文件、原型或预览进行核对。依赖未确认 flow 的 UI 只能保持草稿，不能直接成为稳定实现输入。
...
- 多 agent 协作时，由 coordinator 分配 workset，worker 只写允许范围，提交 `Delta Summary` 和 `Proposed Updates`，最后由 analyze/gate 合并检查，避免互相覆盖。
...
| `/sp.flow`，Codex 中用 `$sp-flow` | 生成或刷新业务流程、状态流、可渲染流程图和核对标签 |
| `/sp.ui`，Codex 中用 `$sp-ui` | 生成或刷新界面、页面映射、表单、交互说明和可核对 UI 预览 |
