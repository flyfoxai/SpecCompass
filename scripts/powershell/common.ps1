...
function Get-FeaturePathsEnv {
    ...
    #   4. Branch-name-based prefix lookup (same as scripts/bash/common.sh)
...
function Get-FeatureSlugFromBranch {
    ...
    $slug = Split-Path (Get-SpecKitEffectiveBranchName $BranchName) -Leaf
...
function Get-FeatureStageLabel {
    ...
    if ((Test-Path -LiteralPath $tasksFile -PathType Leaf) -and -not ((Get-Content -LiteralPath $tasksFile -Raw) -match 'SP_STAGE_SEED: tasks')) { return 'tasks prepared' }

    $planFile = Join-Path $FeatureDir 'plan.md'
    if ((Test-Path -LiteralPath $planFile -PathType Leaf) -and -not ((Get-Content -LiteralPath $planFile -Raw) -match 'SP_STAGE_SEED: plan')) { return 'planning ready' }

    $bundleFile = Join-Path $FeatureDir 'bundle.md'
    if ((Test-Path -LiteralPath $bundleFile -PathType Leaf) -and -not ((Get-Content -LiteralPath $bundleFile -Raw) -match 'SP_STAGE_SEED: bundle')) { return 'bundle ready' }

    $gateFile = Join-Path $FeatureDir 'gate.md'
    if ((Test-Path -LiteralPath $gateFile -PathType Leaf) -and -not ((Get-Content -LiteralPath $gateFile -Raw) -match 'Current Verdict: `Pending Review`')) { return 'gate documented' }

    $clarificationsFile = Join-Path $FeatureDir 'clarifications.md'
    if ((Test-Path -LiteralPath $clarificationsFile -PathType Leaf) -and -not ((Get-Content -LiteralPath $clarificationsFile -Raw) -match 'SP_STAGE_SEED: clarifications')) { return 'clarified' }

    $specFile = Join-Path $FeatureDir 'spec.md'
    if ((Test-Path -LiteralPath $specFile -PathType Leaf) -and -not ((Get-Content -LiteralPath $specFile -Raw) -match 'SP_STAGE_SEED: spec')) { return 'specified' }
    return 'scaffolded'
}
...
function Get-FeaturePrimaryWorkset {
    ...
    $primary = Join-Path $worksetsDir 'ws-primary-journey.md'
    if (Test-Path -LiteralPath $primary -PathType Leaf) { return 'ws-primary-journey' }
