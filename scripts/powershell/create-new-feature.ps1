...
if (-not $DryRun) {
    ...
    $featureTemplateRoot = Resolve-TemplateTreeRoot -TreeName 'feature' -RepoRoot $repoRoot
    if ($featureTemplateRoot) {
        Copy-InstantiatedTemplateTree -TemplateRoot $featureTemplateRoot -TargetRoot $featureDir -BranchName $branchName -FeatureDescription $featureDesc
    }

    if (-not (Test-Path -PathType Leaf $specFile)) {
        $template = Resolve-Template -TemplateName 'spec-template' -RepoRoot $repoRoot
        if ($template -and (Test-Path $template)) {
            Copy-InstantiatedTemplateFile -SourcePath $template -DestinationPath $specFile -BranchName $branchName -FeatureDir $featureDir -FeatureDescription $featureDesc
        } else {
            Write-Warning "Spec template not found; created empty spec file"
            New-Item -ItemType File -Path $specFile -Force | Out-Null
        }
    }

    Write-FeatureContext -RepoRoot $repoRoot -FeatureDir $featureDir
    Sync-ProjectMemory -RepoRoot $repoRoot -ExplicitFeatureDir $featureDir
...
if ($Json) {
    ...
    if (-not $DryRun) {
        Write-Output "# To persist in your shell: `$env:SPECIFY_FEATURE='$branchName'"
    }
}
