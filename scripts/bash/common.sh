...
feature_stage_label() {
    ...
    if [ -f "$feature_dir/tasks.md" ] && ! grep -q 'SP_STAGE_SEED: tasks' "$feature_dir/tasks.md" 2>/dev/null; then
        printf 'tasks prepared'
    elif [ -f "$feature_dir/plan.md" ] && ! grep -q 'SP_STAGE_SEED: plan' "$feature_dir/plan.md" 2>/dev/null; then
        printf 'planning ready'
    elif [ -f "$feature_dir/bundle.md" ] && ! grep -q 'SP_STAGE_SEED: bundle' "$feature_dir/bundle.md" 2>/dev/null; then
        printf 'bundle ready'
    elif [ -f "$feature_dir/gate.md" ] && ! grep -q 'Current Verdict: `Pending Review`' "$feature_dir/gate.md" 2>/dev/null; then
        printf 'gate documented'
    elif [ -f "$feature_dir/clarifications.md" ] && ! grep -q 'SP_STAGE_SEED: clarifications' "$feature_dir/clarifications.md" 2>/dev/null; then
        printf 'clarified'
    elif [ -f "$feature_dir/spec.md" ] && ! grep -q 'SP_STAGE_SEED: spec' "$feature_dir/spec.md" 2>/dev/null; then
        printf 'specified'
    else
        printf 'scaffolded'
    fi
...
feature_primary_workset() {
    local feature_dir="$1"
    local primary_workset="$feature_dir/memory/worksets/ws-primary-journey.md"
    if [ -f "$primary_workset" ]; then
        printf 'ws-primary-journey'
        return
    fi
