...
#
# OUTPUTS:
#   JSON mode: {"FEATURE_DIR":"...", "AVAILABLE_DOCS":["..."], "hasActiveFeature":true/false}
...
if $JSON_MODE; then
    ...
    if has_jq; then
        ...
        jq -cn \
            ...
            '{FEATURE_DIR:$feature_dir,AVAILABLE_DOCS:$docs,hasActiveFeature:true,activeFeature:($feature_dir|split("/")|last),featureDir:$feature_dir,missing:[]}'
    else
        if [[ ${#docs[@]} -eq 0 ]]; then
            json_docs="[]"
        else
        ...
        printf '{"FEATURE_DIR":"%s","AVAILABLE_DOCS":%s,"hasActiveFeature":true,"activeFeature":"%s","featureDir":"%s","missing":[]}\n' \
            "$(json_escape "$FEATURE_DIR")" "$json_docs" "$(json_escape "$(basename "$FEATURE_DIR")")" "$(json_escape "$FEATURE_DIR")"
    fi
else
    # Text output
    echo "FEATURE_DIR:$FEATURE_DIR"
