...
3. Decide the current gate mode and write `<feature_directory>/gate.md`.
   ...
   - When implementation evidence exists, review `Delta Summary` first, then current diff, then task packet, trace/open-items, and necessary source code. Do not treat `Delta Summary` as proof unless it matches current evidence.
   ...
   - Block or route back to `/sp.tasks` when high-risk or code-continuation task packets lack `Read Set`, `Dependencies Checked`, `Reverse Trace Checked`, `Expected Delta`, `Delta Summary`, `Proposed Updates`, or a clear no-applicable reason.
   ...
   - Do not pass when delete, move, rename, public behavior, schema, permission, route, event, or acceptance changes lack reverse-trace/search evidence or an explicit open item.
