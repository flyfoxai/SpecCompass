...
class PresetManifest:
    ...
    def _validate(self):
        ...
        for tmpl in provides["templates"]:
            ...
            if tmpl["type"] == "command":
                # Commands use dot notation (e.g. sp.specify or speckit.ext.command)
                ...
                self._validate_command_namespace(tmpl["name"])
            else:
                if not re.match(r'^[a-z0-9-]+$', tmpl["name"]):
                    raise PresetValidationError(
    ...
    @staticmethod
    def _validate_command_namespace(command_name: str) -> None:
        """Reject preset commands that squat on the core ``sp`` namespace."""
        if core_command_stem(command_name) is not None:
            return

        if command_name.startswith(("sp.", "sp-")):
            allowed = ", ".join(f"sp.{stem}" for stem in sorted(CORE_COMMAND_STEMS))
            raise PresetValidationError(
                f"Invalid command name '{command_name}': "
                "the sp.* and sp-* namespaces are reserved for core commands. "
                f"Use one of the core override names ({allowed}) or an "
                "extension command such as speckit.<extension>.<command>."
            )
...
class PresetManager:
    ...
    def _register_commands(
        ...
        # Extension command names follow the pattern: speckit.<ext-id>.<cmd-name>.
        # Core commands use sp.<cmd-name> (with legacy speckit.<cmd-name> accepted).
        extensions_dir = self.project_root / ".specify" / "extensions"
        filtered = []
        for cmd in command_templates:
            command_id = canonical_command_id(cmd["name"])
    ...
    @staticmethod
    def _skill_names_for_command(cmd_name: str) -> tuple[str, ...]:
        """Return canonical and legacy skill directory names for a command."""
        return skill_directory_variants(cmd_name)

    @staticmethod
    def _skill_title_from_command(cmd_name: str) -> str:
        """Return a human-friendly title for a skill command name."""
        return command_title_text(cmd_name)
    ...
    def _build_extension_skill_restore_index(self) -> Dict[str, Dict[str, Any]]:
        ...
        for _priority, ext_id, _metadata in resolver._get_all_extensions_by_priority():
            ...
            for cmd_info in manifest.commands:
                ...
                for skill_variant in self._skill_names_for_command(cmd_name):
                    restore_index.setdefault(skill_variant, restore_info)
    ...
    def _register_skills(
        ...
        for cmd_tmpl in filtered:
            ...
            command_id = canonical_command_id(cmd_name)
            command_stem = (
                core_command_stem(command_id)
                or command_id.removeprefix("speckit.").replace(".", "-")
            )
            short_name = command_stem.replace(".", "-")
            skill_variants = self._skill_names_for_command(cmd_name)
            ...
            for skill_variant in skill_variants:
                if (skills_dir / skill_variant).is_dir():
                    target_skill_names.append(skill_variant)
            if not target_skill_names and create_missing_skills:
                missing_skill_dir = skills_dir / skill_variants[0]
                if not missing_skill_dir.exists():
                    target_skill_names.append(skill_variants[0])
            ...
            for target_skill_name in target_skill_names:
                ...
                preserved_frontmatter = frontmatter
                if selected_ai in {"codex", "kimi"}:
                    preserved_frontmatter = registrar._filter_skill_frontmatter_variants(
                        preserved_frontmatter, self.project_root
                    )
    ...
    def _unregister_skills(self, skill_names: List[str], preset_dir: Path) -> None:
        ...
        for skill_name in skill_names:
            # Derive command stem from skill directory name.
            short_name = skill_basename_stem(skill_name)
...
class PresetCatalog:
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/flyfoxai/SpecCompass/main/presets/catalog.json"
    COMMUNITY_CATALOG_URL = "https://raw.githubusercontent.com/flyfoxai/SpecCompass/main/presets/catalog.community.json"
