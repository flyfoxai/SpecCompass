...
_TOP_LEVEL_KEYS = frozenset({"schema_version", "workflow", "requires", "inputs", "steps"})
_WORKFLOW_KEYS = frozenset({
    "id", "name", "version", "author", "description",
    "integration", "model", "options",
})
_INPUT_KEYS = frozenset({"type", "required", "default", "prompt", "enum", "description"})
_COMMON_STEP_KEYS = frozenset({
    "id", "type", "command", "prompt", "run", "input", "inputs",
    "integration", "model", "options", "output",
    "condition", "then", "else", "cases", "default",
    "steps", "step", "items", "wait_for", "message", "question",
    "on_reject", "show_file", "expression", "max_iterations",
    "max_concurrency",
})


def validate_workflow_warnings(definition: WorkflowDefinition) -> list[str]:
    """Return non-blocking workflow schema warnings.

    The workflow schema is intentionally open so future hooks can add fields
    without breaking old CLIs. Unknown fields warn instead of failing.
    """
    warnings: list[str] = []

    for key in definition.data:
        if key not in _TOP_LEVEL_KEYS:
            warnings.append(f"Unknown top-level workflow field {key!r}; preserved but not interpreted.")

    workflow = definition.data.get("workflow", {})
    if isinstance(workflow, dict):
        for key in workflow:
            if key not in _WORKFLOW_KEYS:
                warnings.append(f"Unknown workflow metadata field {key!r}; preserved but not interpreted.")

    if isinstance(definition.inputs, dict):
        for input_name, input_def in definition.inputs.items():
            if isinstance(input_def, dict):
                for key in input_def:
                    if key not in _INPUT_KEYS:
                        warnings.append(
                            f"Unknown input field {key!r} on input {input_name!r}; "
                            "preserved but not interpreted."
                        )

    if isinstance(definition.steps, list):
        _collect_step_warnings(definition.steps, warnings)

    return warnings


def _collect_step_warnings(
    steps: list[dict[str, Any]],
    warnings: list[str],
) -> None:
    """Recursively collect unknown-field warnings for workflow steps."""
    for step_config in steps:
        ...
        for key in step_config:
            if key not in _COMMON_STEP_KEYS:
                warnings.append(
                    f"Unknown step field {key!r} on step {step_id!r}; "
                    "preserved but not interpreted."
                )

        for nested_key in ("then", "else", "steps", "default"):
            nested = step_config.get(nested_key)
            if isinstance(nested, list):
                _collect_step_warnings(nested, warnings)

        cases = step_config.get("cases")
        if isinstance(cases, dict):
            for case_steps in cases.values():
                if isinstance(case_steps, list):
                    _collect_step_warnings(case_steps, warnings)

        fan_step = step_config.get("step")
        if isinstance(fan_step, dict):
            _collect_step_warnings([fan_step], warnings)
...
class WorkflowEngine:
    ...
    def validate_warnings(self, definition: WorkflowDefinition) -> list[str]:
        """Return non-blocking workflow validation warnings."""
        return validate_workflow_warnings(definition)
