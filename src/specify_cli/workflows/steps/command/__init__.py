...
from specify_cli.command_names import skill_directory_variants
...
class CommandStep(StepBase):
    ...
    execute the command by name. User-facing instructions should use
    the canonical ``/sp.*`` form; integrations may still map that
    identity to host-specific files internally.
    ...
    def _try_dispatch(
        ...
        The integration's ``dispatch_command`` builds the invocation
        expected by that integration, then executes the CLI
        non-interactively. User-facing documentation should still use
        the canonical ``/sp.*`` form.
        ...
        if project_root is None:
            return None
        if not CommandStep._command_is_installed(project_root, impl, command):
            return None
    ...
    @staticmethod
    def _command_is_installed(
        project_root: Path,
        integration: Any,
        command: str,
    ) -> bool:
        """Return whether the requested command is installed for this integration."""
        try:
            from specify_cli.integrations.base import SkillsIntegration
        except ImportError:
            SkillsIntegration = tuple()  # type: ignore[assignment]

        if isinstance(integration, SkillsIntegration):
            skills_dir = integration.skills_dest(project_root)
            for skill_name in skill_directory_variants(command):
                if (skills_dir / skill_name / "SKILL.md").is_file():
                    return True
            return False

        commands_dir = integration.commands_dest(project_root)
        command_file = commands_dir / integration.command_filename(command)
        return command_file.is_file()
