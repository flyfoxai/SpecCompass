...
class IntegrationBase(ABC):
    ...
    * ``context_file``     — path (relative to project root) of the agent
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        ...
        expects. User-facing documentation should use canonical
        slash commands such as ``"/sp.specify my-feature"``; skills
        integrations may map that identity to their installed skill
        directories internally.

        *command_name* may be the canonical name (``"sp.specify"``), a
        legacy core name accepted for migration (``"speckit.specify"``),
        or a bare stem like ``"specify"``.
        """
        invocation = f"/{slash_command_name(command_name)}"
        if args:
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        *template_name* is the stem of the source file (e.g. ``"plan"``).
        Default: ``sp.{template_name}.md`` for core commands. Subclasses override
    ...
    def process_template(
        ...
        7. Replace ``__CONTEXT_FILE__`` with *context_file*
        ...
        # 3. Extract agent_script command from frontmatter
        ...
        # 7. Replace __CONTEXT_FILE__
        content = content.replace("__CONTEXT_FILE__", context_file)

        # 8. Rewrite paths — delegate to the shared implementation in
...
class TomlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"{command_filename_base(template_name)}.toml"
...
class YamlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"{command_filename_base(template_name)}.yaml"
...
class SkillsIntegration(IntegrationBase):
    ...
    def _legacy_core_skill_dirs(command_name: str) -> tuple[str, ...]:
        """Return legacy skill directory names for a core command."""
        if core_command_stem(command_name) is None:
            return ()
        modern = skill_directory_name(command_name)
        return tuple(
            variant
            for variant in skill_directory_variants(command_name)
            if variant != modern
        )
    ...
    def setup(
        ...
        **opts: Any,
        ...
        for src_file in templates:
            ...
            frontmatter, body = registrar.parse_frontmatter(raw)
            adjusted_frontmatter = registrar._adjust_script_paths(frontmatter)
            processed_body = registrar.resolve_skill_placeholders(
                self.key,
