...
def _install_shared_infra(
    ...
    def merge_tree(
        ...
        dst_root: Path,
        *,
        should_copy: Callable[[Path], bool] | None = None,
    ) -> None:
    ...
    if core and (core / "templates" / "project").is_dir():
        ...
        repo_root = Path(__file__).parent.parent.parent
        project_templates_src = repo_root / "templates" / "project"

    project_specify_templates_src = project_templates_src / ".specify" / "templates"
    ...
    if templates_src.is_dir():
        ...
        def should_copy_template(rel_path: Path) -> bool:
            if rel_path.name == "vscode-settings.json":
                return False
            if any(part.startswith(".") for part in rel_path.parts):
                return False
            if rel_path.parts[0] in {"commands", "project"}:
                return False
            if (project_specify_templates_src / rel_path).is_file():
                return False
            return True

        merge_tree(templates_src, dest_templates, should_copy=should_copy_template)
...
def init(
    ...
    enhancements_title = "Enhancement Commands"
...
def workflow_run(
    ...
    if errors:
        ...
        for err in errors:
            console.print(f"  • {err}")
        raise typer.Exit(1)
    warnings = engine.validate_warnings(definition)
    if warnings:
        console.print("[yellow]Workflow validation warnings:[/yellow]")
...
def workflow_add(
    ...
    def _validate_and_install_local(yaml_path: Path, source_label: str) -> None:
        ...
        from .workflows.engine import validate_workflow, validate_workflow_warnings
