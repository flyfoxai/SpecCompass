...
from .command_names import canonical_command_id, core_command_stem, skill_directory_name
...
REINSTALL_COMMAND = "uv tool install specify-cli --force --from git+https://github.com/flyfoxai/openSpecs.git"
...
class ExtensionCatalog:
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/flyfoxai/openSpecs/main/extensions/catalog.json"
    COMMUNITY_CATALOG_URL = "https://raw.githubusercontent.com/flyfoxai/openSpecs/main/extensions/catalog.community.json"
...
class HookExecutor:
    ...
    def _skill_name_from_command(command: Any) -> str:
        """Map a command id to its generated skill directory name."""
        if not isinstance(command, str):
            return ""
        command_id = command.strip()
        if not command_id:
            return ""
        if core_command_stem(command_id) is not None:
            return skill_directory_name(command_id)
        if command_id.startswith("speckit.") and command_id.count(".") >= 2:
            return skill_directory_name(command_id)
        if command_id.startswith("speckit-") and command_id.count("-") >= 2:
            return skill_directory_name(command_id)
        return ""
    ...
    def _render_hook_invocation(self, command: Any) -> str:
        ...
        if (codex_skill_mode or claude_skill_mode or kimi_skill_mode or cursor_skill_mode) and skill_name:
            return f"/{canonical_command_id(command_id)}"
