...
requires:
  ...
  commands:
    - "sp.tasks"                  # Extension needs tasks command
...
hooks:
  # Hook fired after /sp.tasks completes
  after_tasks:
  ...
  # Hook fired after /sp.implement completes
  after_implement:
...
specify extension catalog add --name "community" \
  https://raw.githubusercontent.com/flyfoxai/SpecCompass/main/extensions/catalog.community.json
...
1. **Core commands unchanged inside SP**: `/sp.tasks`, `/sp.implement`, etc. remain in core
...
```bash
# Old (core command in this SP fork)
/sp.taskstoissues
...
AI agents register both names, so callers can migrate to the alternate alias without relying on deprecated global shortcuts like `/sp.taskstoissues`.
