...
| Skill directories (core built-ins use upstream-style `sp-<name>/SKILL.md`; extensions/presets use `speckit-.../SKILL.md`) | `SkillsIntegration` |
...
|---|---|---|
| `command_filename(template_name)` | Custom file naming or extension | Copilot built-ins → `sp.{name}.agent.md`; extensions → `speckit.<ext>.<name>.agent.md` |
...
Use `sp.<command>` for built-in SP command examples. Extension and preset commands keep the upstream-compatible `speckit.<extension>.<command>` namespace where applicable.
