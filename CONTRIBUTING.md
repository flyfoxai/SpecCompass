...
uv run specify init <temp-dir>/sp-test --integration <agent> --offline
cd <temp-dir>/sp-test
...
4. **Run prerequisites first** — commands that depend on earlier commands (e.g., `/sp.tasks` requires `/sp.plan` which requires `/sp.specify`) must be run in order.
...
- src/specify_cli/*.py → CLI commands (`specify init`, `specify check`, `specify extension *`, `specify preset *`); test the affected CLI command and, for init/scaffolding changes, at minimum test /sp.specify
...
- T1: /sp.command — (reason)
- T2: /sp.command — (reason)
~~~
