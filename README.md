...
- `/sp.flow` builds the business/architecture flow baseline and starts a confirmation process when the flow is new, high-risk, broad, or contains decision points. It writes `specs/<feature>/flows/review/flow-review-data.json` and opens it through a Web SpecCompass review page, where reviewers can inspect intuitive flow diagrams, module summaries, short flow explanations, and node-level checkpoints instead of reading raw files.
- In the `/sp.flow` review page, the model prepares plain-language decision cards with a recommended option, alternatives, expected consequences, and the reason for the recommendation. Reviewers use the right-side feedback/confirmation rail to accept the recommendation or submit a different choice with comments. Browser clicks are only local drafts; downstream commands may treat the flow as authorized only after the summary is written to `flow-confirmation.md`.
- `/sp.ui` runs after the relevant flow is confirmed. It turns the flow-backed screen needs into reviewable UI previews, including pages, regions, components, states, and visible interaction decisions. When UI review is needed, it writes independent `specs/<feature>/ui/review/ui-review-data.json`; UI review data is not flow data and must not reuse flow nodes as UI placeholders.
- In the `/sp.ui` review page, reviewers see the interface preview through the same SpecCompass Web review surface, styled by Huashu design rules and paired with the right-side confirmation rail. The model provides recommended UI choices and alternatives for decisions such as layout, key controls, states, and exception handling. UI choices become stable implementation input only after the confirmation result is written to `ui-confirmation.md`.
