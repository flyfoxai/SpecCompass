...
7. Execute implementation following the selected task plan:
   ...
   - **Dependency surface check**: Before editing existing code, satisfy the task's `Dependencies Checked` field or state why a listed dependency is not applicable. Check only direct neighbors first: imports, calls, routes, contracts, schemas, permissions, tests, and workset trace rows. Expand beyond direct neighbors only when evidence shows wider impact.
   - **Data-linkage check**: When the task changes a data object, table, field, state, permission, event, persistence behavior, API contract, UI field, acceptance path, or test semantic, check the direct neighbor flow node, UI surface, API, permission rule, emitted side effect, acceptance path, test, trace row, and open item before claiming completion. Keep the check to direct neighbors unless current evidence shows a wider dependency.
   ...
   - **code handoff consumption**: If this task was created from document-stage discovery, confirm the code handoff packet names target file or directory, reason, related anchor, `Allowed Write Set`, `Required Checks`, expected verification, writeback target, and next route. If those fields are missing, return `NEEDS_TASKS`; if the write boundary is wrong, return `NEEDS_PLAN`.
   ...
   - **Controlled multi-agent default**: Multi-agent execution is an optimization, not the normal path. Use it only for independent worksets, disjoint write sets, bounded context pressure, or explicit user/coordinator delegation that still satisfies the same hard gates. Routine single-file work, coupled refactors, shared schema/registry work, and unclear boundaries stay single-agent sequential.
   - **Pre-dispatch hard gates**: Before delegating worker tasks, the coordinator must have a baseline snapshot, clean-state record, current git `HEAD`, named branch/worktree ref, or equivalent pre-dispatch evidence; explicit disjoint `Allowed Write Set` values; explicit `Required Checks`; satisfied dependencies; and no shared truth or global registry-like worker writes. If a gate is uncertain, do not dispatch; run the work sequentially or route to `/sp.tasks` / `/sp.plan`.
   - **Shared memory coordination**: Parallel tasks [P] may implement independent files or scopes concurrently, but shared writeback must be serialized or batched. Use the canonical shared truth file list and canonical fallback report fields from `sp-command-spec.md` §10.3. Do not let parallel workers simultaneously edit `tasks.md`, `FEATURE_DIR/memory/open-items.md`, `FEATURE_DIR/memory/trace-index.md`, workset routing, analysis/gate state, or broad status summaries. Each worker should report evidence and requested memory changes; one owner step merges shared memory and task-state updates.
   - **Parallel agent boundaries**: When delegating parallel work, state each worker's disjoint write set and mark shared memory files as read-only for that worker. Workers should return evidence and proposed shared updates, not apply those shared updates directly. Use the canonical worker handoff fields from `sp-command-spec.md` §10.3 when assembling the handoff.
   ...
   - **Worker handoff**: A delegated worker must return a concise handoff instead of editing shared memory directly. Include `Task / Workset`, `Status`, `Execution Environment`, `Allowed Write Set`, `Actual Files Changed`, `Anchors Affected`, `Inputs Read`, `Checks Run`, `Result`, `Evidence`, `Proposed Shared Updates`, `Open Items / Risks`, and `Merge Notes`.
   - **Fallback classification**: If observable evidence from worker handoffs, command output, branches/worktrees, current diffs, or coordinator records shows that worker execution failed, timed out, returned empty output, changed files outside the allowed write set, touched shared/global files without authorization, missed evidence, conflicted with another worker, or depended on a worker that does not satisfy all dependency-closure requirements (merged, independently verifiable, and classified `ACCEPTABLE_LOCAL`), freeze the batch: stop dispatching new worker work, stop merging unverified worker output, classify all worker results, preserve evidence, and recover remaining or unclear work sequentially. Classify workers as `ACCEPTABLE_LOCAL`, `NEEDS_SINGLE_AGENT_REVIEW`, `REJECTED_BOUNDARY_VIOLATION`, `STALE`, or `FAILED_CHECKS`.
   - **Single-agent recovery**: When multi-agent coordination becomes the risk source, downgrade to single-agent sequential recovery. Re-read the current task packet, source-of-truth docs, memory, trace/open-items, current diff, and worker handoffs as evidence only. Use the baseline evidence to isolate or clean unverified and unauthorized worker diffs from the recovery path before ingesting acceptable results. Do not run destructive cleanup in a user workspace unless explicitly approved; if cleanup overlaps user-owned changes or cannot be made safe, stop with `BLOCKED` or `NEEDS_DECISION` and present cleanup options. Ingest only independently verifiable local results and run current required checks before closing each unit.
   ...
   - Keep `Root Layer` and `Next Route` consistent. Route data schema/planning/migration-design issues to `/sp.plan` or `/sp.flow`, data compatibility or initialization choices to `/sp.clarify`, and pure fixture/script syntax errors to `/sp.implement`.
   ...
   - A second attempt on the same failure signature must cite concrete first-attempt failure evidence. If that evidence is unavailable, stop with `BLOCKED` and route to the owner command instead of retrying from memory.
   ...
   - When the same failure signature appears twice in the same workset, the fallback blocks stage entry, or the issue involves human decision, data migration, permissions, security, release, rollback, or worktree cleanup, append a fallback-log entry or `promote-candidate: <Failure Signature>` for `/sp.analyze` or `/sp.gate`. Do not directly create, merge, close, or promote `FEATURE_DIR/memory/open-items.md` blocker state from `/sp.implement`.
   ...
   - Ask humans only after bounded rule/document/test diagnosis shows the choice is genuinely about product scope, risk, compliance, rollback, destructive action, or incompatible requirements. Otherwise use the constitution, source docs, tasks, trace, tests, and current evidence to repair or route the problem.
   - When `NEEDS_DECISION` is reached, freeze downstream work for the same `Blocker ID`: stop implementation, route to `/sp.clarify` or ask directly with background, impact, 2-4 options, tradeoffs, recommendation, and next `/sp.*` route. Resume only after the human-selected decision is written back to the source doc, task, or `FEATURE_DIR/memory/open-items.md`.
   ...
   - **Reverse trace check**: Before delete, move, rename, public behavior, schema, permission, route, event, or acceptance changes, satisfy the task's `Reverse Trace Checked` field or record the reference scan performed. Use trace rows, text search, imports/calls, routes, tests, same-directory references, and registered contracts as applicable. If the reverse lookup is missing or ambiguous for a high-risk object, stop with `NEEDS_TASKS`, `NEEDS_PLAN`, or `BLOCKED` rather than guessing.
...
8. Progress tracking and error handling:
   ...
   - For parallel tasks [P], if observable worker/coordinator evidence shows a worker failed or became unverifiable, freeze the batch, classify workers, keep only independently verifiable `ACCEPTABLE_LOCAL` results, and use single-agent recovery for the rest.
   ...
   - For multi-agent fallback, persist task-local reports in the relevant `tasks.md` task note and batch-level reports in the coordinator closeout output before claiming the batch is closed. Use these fields: `Fallback Reason`, `affected worker classifications`, `changed files`, `evidence kept`, `discarded/deferred results`, `single-agent recovery route`, and `next /sp.* step`. If the fallback is repeated, upward, stage-blocking, or involves human/data/permission/security/release/rollback/worktree cleanup risk, also append or propose `FEATURE_DIR/memory/fallback-log.md` loop evidence without treating it as blocker truth.
   ...
   - For code-continuation tasks, include a concise `Delta Summary` before claiming completion: expected delta, actual files changed, anchors affected, dependency checks performed, reverse-trace result, checks run, proposed shared updates, and remaining gaps. This summary is the first review surface for `/sp.analyze`, `/sp.gate`, human review, or a coordinator; do not replace it with a broad prose progress report.
   ...
   - **IMPORTANT** For every task whose own completion conditions are met, update the task state in the same execution:
     ...
     - when closing, deleting, accepting, deferring, downgrading, or invalidating `Risk`, `Blocker`, High severity item, broader-impact item, or `@r0`, record `Close Evidence`: the current evidence, traceable code/doc change, rollback/degrade path, or explicit human acceptance that justifies it
...
9. Completion validation:
   ...
   - Confirm memory/source-doc writeback is complete when stable facts changed:
     ...
     - direct data-linkage evidence changed across flow, UI, API, data, permissions, events, acceptance, tests, trace, or open items
   ...
   - Confirm each code-continuation or high-risk task has a filled `Delta Summary` and any `Proposed Updates` needed for shared memory, trace, open-items, source docs, or task state. If the worker is not allowed to write shared files, leave the proposed updates for coordinator closeout rather than editing shared state directly.
...
End every run with a concrete closeout recommendation. Do not only report implementation completion, failed checks, or generic next steps. Give 2-3 options, choose one, explain why, and provide a one-line copy-pasteable `NEXT_COMMAND`.

Before choosing the recommendation, reconcile `.specify/memory/active-context.md`, `.specify/memory/feature-map.md`, feature `memory/index.md`, feature `memory/open-items.md`, selected task packet, Delta Summary, checks run, task-state writeback, and this implementation evidence. If checks failed, writeback is incomplete, risk acceptance is missing, or the task boundary was wrong, recommend `/sp.tasks`, `/sp.plan`, `/sp.clarify`, `/sp.analyze`, or the exact owner route instead of claiming downstream readiness.
...
- Recommend `/sp.analyze <feature>` after implementation only when selected task evidence, checks, task-state writeback, and open-item handling are complete enough for diagnostic review.
- Recommend `/sp.tasks`, `/sp.plan`, `/sp.clarify`, or the exact owner route when implementation exposed task-packet, plan, risk, human-decision, or boundary problems.
- Do not recommend another `/sp.implement` unless the next ready `Mode: impl` task is clearly authorized and no analyze/gate review is required by the current workflow.
