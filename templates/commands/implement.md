...
9. Completion validation:
   ...
   - If the user asks `/sp.implement` to "solve blockers", execute only the selected ready `Mode: impl` repair task. After local evidence is recorded, route unresolved or cross-layer blocker closeout to `/sp.analyze`, `/sp.gate`, or `/sp.clarify` instead of declaring all blockers closed from implementation prose.
