...
3. Generate or refresh `specs/<feature>/tasks.md`.
   ...
   - For tasks that need human decision, risk acceptance, split approval, or downgraded verification, write the task so the model can ask in plain language with background, impact, 2-4 options, tradeoffs, recommendation, and next `/sp.*` route.
