...
6. Create or refresh `specs/<feature>/spec-outline.md` readiness.
   ...
   - `specify_ready` is Level 3 structural completion. Enter it only when the Level 1 minimum and the Level 2 product boundary are confirmed and source-backed enough for formal graphical confirmation. The model may organize confirmed product facts and check structural completeness, but it must not invent target users, product goals, business rules, or scope, and it must not use the Constitution to infer, complete, or decide product content. Constitution content remains display-only here; route formal governance decisions to `/sp.constitution`.
   ...
   - Build Level 1 and Level 2 Discovery in two explicit stages. Do not draft map titles or generic overview copy before Stage A is complete.
   - **Stage A - extract business semantics**: derive `business_context` only from the user input, current PRD, accepted intent-ledger events, and formal business sources. Record the real `product_subject`, `business_objects`, `operations`, `outcomes`, `business_chains`, and `evidence_gaps`. Each complete business chain must connect a trigger or input, at least one business object, an operation or control, and an observable outcome, with `source_refs`. Model additions remain `ai-proposed`.
   - **Stage B - compile the business maps**: use the complete business chains to name the Overview root, summarize the real business loop, and create up to three cohesive business capability branches. A Level 1 branch must say what the product stores, decides, controls, exchanges, delivers, or verifies; generic branches such as goals, users, problems, scope, project understanding, or methodology are not business capability branches. Put supporting roles, scenarios, boundaries, and questions under the business branch they affect.
   - `frame` additionally requires at least one complete business chain backed by a user-confirmed or formal-document source. An `ai-proposed` chain may be shown as a candidate but cannot advance maturity. If no source-backed chain exists, stay in `explore`, bind the missing object, operation, outcome, or source to a concrete branch question, and never fill the map with generic process language.
   ...
   - Bind every discovery question to one concrete `outline_node_id`. The selected node is the unit of discussion: its 2-4 business `candidates`, single recommendation with a reason, none of the above path, and free-form input must resolve or extend that branch. Every candidate must include `business_chain_refs` that point to `business_context.business_chains`; a recommendation without business-chain provenance is invalid. Do not place branch-specific choices in an unrelated global list, and do not reuse formal confirmation `options` or their routing semantics.
