...
- If `spec-outline.md` is `READY_FOR_SPECIFY`, suggest `/sp.specify`.
- If readiness is `NEEDS_PRD`, suggest another `/sp.prd` pass with the missing interview areas.
- If readiness is `NEEDS_SOURCE`, ask for source recovery or an explicit rebase decision before `/sp.specify`.
- If readiness is `SPLIT_REQUIRED`, ask the user to confirm the split before `/sp.specify`.
