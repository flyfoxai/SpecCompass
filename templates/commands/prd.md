...
6. Create or refresh `specs/<feature>/spec-outline.md` readiness.
   ...
   - For `explore` and `frame`, generate `specs/<feature>/prd/review/outline-discovery-data.json` with `interaction_mode: discovery`. Each high-value question has 2-4 business `candidates`, one recommendation with a reason, a none of the above path, and free-form input. Do not reuse formal confirmation `options` or their routing semantics.
