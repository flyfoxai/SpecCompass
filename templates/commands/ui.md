...
## Default UI Regeneration

An ordinary `/sp.ui <feature>` means: discard every active UI artifact previously generated by `/sp.ui` for that feature and use model reasoning to regenerate the UI from the effective PRD source corpus, the current confirmed Outline, the current confirmed Flow, stable Spec/clarifications, and design-authority facts. Do not patch or paraphrase the old UI.

Here, ordinary means a direct full-stage invocation: not `--consume-review-confirmation`, not a transition-specific impact handoff, and not a coordinator-authorized Lite scoped pass. Complete every read-only prerequisite, active-Lite, and route check described below before calling reset `apply`. If any check stops or narrows the run, leave active UI artifacts untouched. Section order does not authorize deleting before those checks finish.
...
Run `node .specify/review/scripts/reset-command-artifacts.mjs inspect ui specs/<feature>`. For `CLEAR_AND_REGENERATE`, immediately apply `--mode clear --inventory-digest <digest>`; the ordinary command already requests replacement of unconfirmed generated output. For `CONFIRMED_RECORDS_REQUIRE_CHOICE`, ask only whether to preserve confirmed records for fresh review or clear everything. Use `--mode preserve-confirmed` for the former and `--mode clear --ack-confirmed` for the latter. Do not choose for the user. `NO_GENERATED_ARTIFACTS` needs no reset.

The reset owns `ui/` output, including screen documents, maps, schemas, examples, review data, batches, decisions, confirmations, previews, and prior command history; it does not own PRD, Spec, Flow, plan, tasks, code, tests, data, or migrations. Preserved records are copied to `ui/review/history/regeneration-preserved/<inventory>/` as non-authoritative candidates; they cannot be consumed through `--consume-review-confirmation`, and regenerated output requires fresh confirmation. Clear mode must not cite old UI decisions. Regenerate the full currently applicable UI set, remove obsolete screens by omission, write complete review data, and finish the normal UI review path. A missing boundary advisory may be mentioned briefly after the UI result but cannot block it.

A transition-specific UI impact pass is different from ordinary regeneration and is allowed only when `/sp.prd` explicitly hands off the same active `transition_id`; it may record `UNCHANGED_WITH_EVIDENCE`, `REGENERATE`, `MIGRATE`, `RETIRE`, or `BLOCKED`, but cannot activate the baseline or perform unrelated UI work. Never directly edit derived boundary fields in `review-index.json`.
...
- Treat repository-root `prd/` as the default business-source corpus. Add any human-specified source directories unless the human explicitly replaces the default. Record the effective roots and exact anchors in the UI source snapshot.
- Stable UI input is `PRD sources + confirmed Outline + confirmed Flow + stable Spec/clarifications + design authority`. No old UI draft may replace those inputs.
...
- Read `specs/<feature>/prd.md`, `specs/<feature>/spec-outline.md`, its current Outline confirmation/readiness evidence, `specs/<feature>/spec.md`, `specs/<feature>/clarifications.md`, `specs/<feature>/flows/*`, and the current Flow confirmation/readiness evidence.
...
- Confirm routing identifies one active feature, the Outline is currently confirmed and not stale, and `spec.md` plus the required confirmed Flow contracts are current enough to define UI facts.
...
- Check whether the requested UI decision depends on unresolved layout choice, interaction model, data binding, validation, permission, risk acceptance, safety, money, compliance, irreversible behavior, critical acceptance, or broad downstream impact. Resolve low-impact reversible UI choices with model judgment and record them. If a material choice belongs to UI and can be represented by UI review data, generate a bounded draft and put 3-4 executable options, impacts, and one recommendation in a `must_confirm` node on the UI Web review; keep readiness blocked until mechanical human writeback. Route to `/sp.flow`, `/sp.specify`, or `/sp.clarify` only when the missing fact belongs upstream or cannot be represented by the UI review contract.
...
- Confirm whether the current run belongs to an existing UI batch or starts a
  ...
  evidence.
- If the flow contract and business domain are clear but UI information is coarse, continue with a bounded draft UI decomposition. Use common product UI patterns to fill low-impact reversible screens, states, fields, and actions, mark each inferred element as `Source: model-inferred`, and keep it draft until review or later checks. Put UI-owned material choices in the UI Web review; route missing business rules, Flow facts, permissions, validation authority, or acceptance meaning to their upstream owner instead of inferring them.
...
- If coarse input admits several low-impact reversible UI patterns, choose the simplest source-compatible pattern, mark the inference, and continue. If alternatives materially change authority, risk, acceptance, workflow, or downstream scope, put 3-4 distinct options and a recommendation in the UI Web review rather than choosing silently or asking only in chat.
...
- If the UI decision depends on unresolved scope, Flow, permission, or acceptance behavior, fall back to `/sp.prd`, `/sp.flow`, `/sp.specify`, or `/sp.clarify` instead of inventing the screen behavior. UI-owned material choices stay in the UI Web review.
...
- If the same UI issue has multiple reasonable layouts, interaction models, or information-architecture repairs with material impact, encode 3-4 options with impact and recommendation in the current UI Web review. Choose ordinary reversible repairs autonomously and record them.
...
- When UI review data is created or repaired, change only the current real
  ...
  adoption, or block UI generation. `migrate-review-index.mjs` remains a legacy
  diagnostic and is not a regeneration prerequisite. A real active transition
  or invalid established authority blocks.
- With registered authority, derive UI `modules[]` and screens inside the
  confirmed project boundary. With unregistered legacy authority, derive them
  directly inside the explicitly targeted feature from its PRD, Spec, Flow,
  responsibilities, and business chains without asserting parentage.
  Analytical Outline nodes keep `outline_node_id` identity but do not dictate
  UI module count. A registered stable
  ...
  blocked by preflight. During an approved migration, classify each affected UI object as
...
- Do not read active `ui-confirmation.md` as generation authority before the
  ...
  regenerate the full currently applicable UI scope and require fresh
...
- After the user completes batch confirmation, the served review page writes
  ...
  record operation and invokes no model capability. A fully `CONFIRMED` result
  with no revision/open items returns `/sp.ui <feature>
  ...
  ordinary `/sp.ui <feature>`, enters the inspected preserve-or-clear choice
  before full regeneration. Accept a downloaded package or copied summary only as an
