...
An ordinary `/sp.ui <feature>` means: discard every active UI artifact previously generated by `/sp.ui` for that feature and use model reasoning to regenerate the UI from current PRD, Spec, Flow, clarification, and design-authority facts. Do not patch or paraphrase the old UI.
