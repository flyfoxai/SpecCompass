...
- When flow/UI uses `confirm_strategy: batch`, require `Batch ID`, `Batch Scope`, `Batch Review Status`, and the relevant confirmation document (`flow-confirmation.md` or `ui-confirmation.md`) before PASS. Review manifests, screenshots, HTML local state, or browser state are auxiliary review evidence only and cannot replace the authorization document. `WAITING_FOR_BATCH_REVIEW`, partial, rejected, or stale batch status cannot support PASS, CONDITIONAL, risk closure, trace closure, or implementation readiness unless the unresolved part has been explicitly split into a child batch and dependencies are recorded.
