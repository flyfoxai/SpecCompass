...
Before ordinary specification work, resolve the one root feature from the explicit target and `specs/review-index.json`; never infer ancestry from numeric codes or array order. Run `node .specify/review/scripts/check-outline-boundary-gate.mjs specs/<root-feature>/outline-boundaries.json specs/review-index.json --feature <feature> --stage specify`. Accept only schema `speccompass.outline-boundary-gate.v1` with `allowed: true`, `transition_state: ALIGNED`, and the requested feature in the current baseline. `PORTFOLIO_ROOT_NOT_IMPLEMENTATION_TARGET` is final for this route: select an implementation child; never create `spec.md` in the root.
...
- Treat `/sp.specify` as the stable implementation-facing specification compiler and baseline specification point. `/sp.prd` is the mandatory upstream requirement intake for new feature work; `prd.md` is the requirement-source container. User-confirmed, formally sourced, and consumed-decision content is requirement fact, while model proposals and explicitly uncertain material remain candidates. `/sp.specify` carries only eligible facts into `spec.md`.
