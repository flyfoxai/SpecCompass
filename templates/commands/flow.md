...
An ordinary `/sp.flow <feature>` means: discard every active Flow artifact previously generated by `/sp.flow` for that feature and use model reasoning to regenerate the Flow from current PRD, Spec, clarification, and other upstream facts. Do not patch or paraphrase the old Flow.
