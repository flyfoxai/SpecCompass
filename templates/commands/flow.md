...
An ordinary `/sp.flow <feature>` means: discard every active Flow artifact previously generated by `/sp.flow` for that feature and use model reasoning to regenerate the Flow from the effective PRD source corpus, the current confirmed Outline, stable Spec, clarifications, and other upstream facts. Do not patch or paraphrase the old Flow.

Here, ordinary means a direct full-stage invocation: not `--consume-review-confirmation`, not a transition-specific impact handoff, and not a coordinator-authorized Lite scoped pass. Complete every read-only prerequisite, active-Lite, and route check described below before calling reset `apply`. If any check stops or narrows the run, leave active Flow artifacts untouched. Section order does not authorize deleting before those checks finish.

The exact `/sp.flow <feature> --consume-review-confirmation` route is not an ordinary regeneration. It exists only for the fixed loopback writer's fully `CONFIRMED` result. Do not clear or regenerate. Recompute the complete Review Data ID, validate feature, batch, source identity, owner approval, and empty revision/open/draft-excluded lists, then record the current Flow as ready for UI/downstream use. Any mismatch or non-confirmed status fails closed and returns ordinary `/sp.flow <feature>` for fresh generation; never consume a preserved archive or copied/model-authored confirmation through this route.

Before clearing anything, resolve the explicit feature and root and run `node .specify/review/scripts/check-outline-boundary-gate.mjs specs/<root-feature>/outline-boundaries.json specs/review-index.json --feature <feature> --intent regenerate --stage flow`. Accept only schema `speccompass.outline-boundary-gate.v1`. Missing legacy boundary registration is a non-blocking advisory only for a non-root feature; `PORTFOLIO_ROOT_NOT_IMPLEMENTATION_TARGET` always blocks root-level Flow generation and consumption. A valid human-approved structural transition, unreadable/invalid established authority, interrupted finalization, or a target excluded by an established baseline still blocks before clearing. A derived index mismatch may be repaired mechanically with the returned sync command and then rechecked.
...
Run `node .specify/review/scripts/reset-command-artifacts.mjs inspect flow specs/<feature>`. For `CLEAR_AND_REGENERATE`, immediately apply `--mode clear --inventory-digest <digest>`; the ordinary command already requests replacement of unconfirmed generated output. For `CONFIRMED_RECORDS_REQUIRE_CHOICE`, ask only whether to preserve confirmed records for fresh review or clear everything. Use `--mode preserve-confirmed` for the former and `--mode clear --ack-confirmed` for the latter. Do not choose for the user. `NO_GENERATED_ARTIFACTS` needs no reset.

The reset owns `flows/` output, including diagrams, index, review data, batches, decisions, confirmations, and prior command history; it does not own PRD, Spec, UI, plan, tasks, code, tests, data, or migrations. Preserved records are copied to `flows/review/history/regeneration-preserved/<inventory>/` as non-authoritative candidates; they cannot be consumed through `--consume-review-confirmation`, and regenerated output requires fresh confirmation. Clear mode must not cite old Flow decisions. Regenerate the full currently applicable Flow set, remove obsolete diagrams by omission, write complete review data, and finish the normal Flow review path. A missing boundary advisory may be mentioned briefly after the Flow result but cannot block it.

A transition-specific Flow impact pass is different from ordinary regeneration and is allowed only when `/sp.prd` explicitly hands off the same active `transition_id`; it may record `UNCHANGED_WITH_EVIDENCE`, `REGENERATE`, `MIGRATE`, `RETIRE`, or `BLOCKED`, but cannot activate the baseline or perform unrelated Flow work. Never directly edit derived boundary fields in `review-index.json`.
...
- Treat repository-root `prd/` as the default business-source corpus. Add any human-specified source directories unless the human explicitly replaces the default. Record the effective roots and exact anchors in the Flow source snapshot.
- Confirmed parent handoff refs, current feature sources, and user-named files are priority reads, not an allowlist. Flow must search the authorized source roots for facts relevant to the current boundary.
- Stable Flow input is `PRD sources + confirmed Outline + stable Spec/clarifications`. None of these may be replaced by an old Flow draft.
...
`/sp.flow` run first applies the command-artifact preserve-or-clear choice. In
preserve mode, verify that the confirmation batch and Review Data ID match the
archived record, then use every `target_ref` and human note as non-authoritative
candidate input while regenerating the complete current Flow and complete fresh
review data. In clear mode, do not read or reconstruct those requests. A
revision request is not authorization.
...
- Read the relevant files under repository-root `prd/` and every human-specified source root.
- Read `specs/<feature>/prd.md`, `specs/<feature>/spec-outline.md`, its current Outline confirmation/readiness evidence, `specs/<feature>/spec.md`, `specs/<feature>/clarifications.md`, and rule lists that affect the target flow.
...
- Confirm routing identifies one active feature, the Outline is currently confirmed and not stale, and `spec.md` is current enough to define flow facts.
...
- Check whether the requested flow decision depends on unresolved user intent, conflicting requirements, risk acceptance, verification downgrade, permission ownership, safety, money, compliance, irreversible behavior, critical acceptance, or broad downstream impact. If the decision belongs to Flow and can be expressed by Flow review data, generate a bounded draft and put 2-4 executable options, impacts, and one recommendation in a `must_confirm` node on the Flow Web review; keep readiness blocked until mechanical human writeback. Route to `/sp.prd`, `/sp.specify`, or `/sp.clarify` only when the missing decision belongs upstream or cannot be represented by the Flow review contract.
...
- If the source information is coarse but the business domain, user role, business goal, and feature boundary are clear, continue with a bounded draft decomposition. Use common business-flow patterns to fill low-impact reversible details, mark each inferred step as `Source: model-inferred`, and keep it draft until review or later checks. If the coarse source hides a Flow-owned material choice, expose it as a Web `must_confirm` target; if it hides an upstream requirement or project-boundary fact, route to that upstream owner instead of inferring it.
...
- If coarse input admits several low-impact reversible flow arrangements, choose the simplest source-compatible one, mark the inference, and continue. If alternatives materially change business behavior or downstream scope, put 2-4 distinct options and a recommendation in the Flow Web review rather than choosing silently or asking only in chat.
...
- If branch, state, or exception behavior cannot be resolved after bounded evidence review, use a Flow Web decision target when Flow owns the choice; otherwise fall back to `/sp.prd`, `/sp.specify`, or `/sp.clarify` instead of inventing the transition.
...
- If the same flow issue has multiple reasonable repairs with material impact, encode 2-4 options with impact and recommendation in the current Flow Web review. Choose ordinary reversible repairs autonomously and record them.
...
- When flow review data is created or repaired, change only the current real
  ...
  `has_outline_discovery`. When boundary authority is registered and aligned,
  rebuild derived `feature_code`, `order`,
  ...
  `validate-review-index.mjs` and the regeneration-intent boundary gate. When
  authority is unregistered, preserve all identity fields and update the flag
  only if an existing schema-v2 entry remains valid without identity changes.
  If the legacy index cannot validate, leave it unchanged and report a
  non-blocking navigation/index advisory; do not fabricate hierarchy, force
  adoption, or block Flow generation. `migrate-review-index.mjs` remains a
  legacy diagnostic and is not a regeneration prerequisite. A real active
  transition or invalid established authority blocks.
- With registered authority, derive flow `modules[]` inside the confirmed
  project boundary. With unregistered legacy authority, derive them directly
  inside the explicitly targeted feature from its PRD, Spec, responsibilities,
  business objects, and business chains without asserting parentage.
  Analytical Outline nodes keep `outline_node_id` identity but do not dictate
  Flow module count. A registered stable
  boundary remains `outline_alignment: one_to_one`; a real `merged`, `split`,
  `diverged`, or `not_mapped` transition is not the missing-legacy case and is
  blocked by preflight. During an approved migration, classify each affected Flow object
  as `UNCHANGED_WITH_EVIDENCE`, `REGENERATE`, `MIGRATE`, `RETIRE`, or `BLOCKED`.
- Do not read active `flow-confirmation.md` as generation authority before the
  reset choice. After `preserve-confirmed`, verify and read only the returned
  preservation manifest and archived records as non-authoritative candidates;
  regenerate the full currently applicable Flow scope and require fresh
  confirmation. After clear mode, no old confirmation or revision request may
...
- After the user completes batch confirmation, the served review page writes
  ...
  mechanical record operation and invokes no model capability. A fully
  `CONFIRMED` result with no revision/open items returns `/sp.flow <feature>
  --consume-review-confirmation`; that exact route only revalidates the active
  identity and advances readiness. A revision/open result, or a manually run
  ordinary `/sp.flow <feature>`, enters the inspected preserve-or-clear choice
  before full regeneration. Accept a downloaded package or copied summary only
...
- Present flow review data with the fixed renderer main entry
  ...
  `/sp.accept flow <feature>` command, which is an explicit blanket
...
- Confirm review data marks which nodes need feedback by default / 节点反馈默认折叠,
  ...
  browser reset semantics are described as clearing only current-view draft
  choices. Keep that browser action distinct from command-level regeneration,
  which may clear `flow-confirmation.md` only through the inspected user-choice
  contract above.
