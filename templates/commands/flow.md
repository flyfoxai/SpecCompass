...
Historical Regressions`, `Global Status`, and `Blocker Route` before changing
flow artifacts. Run the platform-appropriate installed `sp-lite-state` script
with JSON output and accept only schema `speckit.lite.route.v1`.

Normal Lite flow work is authorized only when the fresh payload has
`globalControl=CLEAR`, `continueAllowed=true`, and `next="/sp.flow"`. On any
other normal route, stop without writing and return its `next`. A non-`CLEAR`
route is resolution-only: proceed only when `Blocker Route` is `/sp.flow` and
...
Work only on the included anchors, never pull deferred anchors into the round,
...
Label every Lite-scoped flow writeback with `Lite Round`. Human confirmation
may use `SCOPED_CONFIRMATION` only for the included anchors; it does not confirm
deferred anchors or the full feature.

Before returning a completed Lite stage, publish round evidence in
`flows/index.md` with these exact fields: `Lite Round`, `Lite Stage`,
`Included Outline Anchors`, and `Source Signature`. Set the stage to `FLOW` and
use the before-dispatch signature supplied by the coordinator. A reviewed Lite
flow must also record `Human Confirmation: CONFIRMED`; otherwise return the
review route and do not claim completion.
...
- If the flow draft contains a human decision point, explain the real business
  background in plain Chinese (the real business background, not a schema or
  trace description), give tiered decision options (`must_confirm` defaults to
  3-4 options; a source-backed mutually exclusive binary decision may use 2
  options when `options_count_rationale` explains why no third executable exit
  exists; ordinary human-judgment nodes default to 3 options), and make every
  option a business-readable executable exit. Each decision node must provide
  reviewer-facing `背景信息` / `decision_background` and `决策摘要` /
  `decision_summary` so the reviewer knows what is being decided and why it
  matters now. Each option must expose `收益` / `benefit` and `代价` / `cost`;
  the recommended option must also expose `推荐理由` /
  `recommendation_reason`. The execution fields / 执行字段 `consequence` and
  `next_exit` remain mandatory for writeback and downstream routing, but they
  are not the primary visible decision explanation. Do not use reusable 模板句 /
  ...
  不选推荐 through concrete cost text, and keep 真实差异 between options; do not disguise the same route as
...
decision_records:
  - id: <visible label or stable ID>
    ...
    benefit: <what this choice gains for the business, review, delivery, or risk>
...
- Before writing a review-data node, turn that port contract into a human business sentence. Every business node must make clear who acts or is responsible, what business situation or trigger starts the step, what action or judgment happens, what state/result changes, and who continues the work or handles recovery. Do not use a label plus a generic sentence such as “进入下一步” or “系统处理数据” as the node explanation; the reviewer must understand why this step exists in the end-to-end business outcome.
- For every `decision` or `human_judgment` node, label every outgoing edge with the business condition, decision result, or recovery reason that sends work to the target node. Labels such as “继续”, “下一步”, “处理”, or “完成” are not sufficient. Preserve the real two-way split when the source only supports two exits; do not invent a third option or branch merely to satisfy a count.
...
- Present flow review data with the fixed renderer main entry
  ...
  business overview / 项目整体业务地图, review goal / 本轮审核目标, module summary / 模块简介, per-flow summary
  ...
  `options_count_rationale` whenever a source-backed binary choice uses only 2
  options, including a `must_confirm` decision.
  The project overview and review goal, each module summary, and each flow summary must be substantive business context rather than layout or process boilerplate; the validator rejects generic copy before the renderer receives it. Also provide batch scope, pending-decision routes, blocker/stale reasons, and
  writeback target `flow-confirmation.md`.
  `must_confirm` nodes default to 3-4 options; a source-backed mutually
  exclusive binary decision may use 2 options only with
  `options_count_rationale`; ordinary human-judgment nodes default to 3 options.
  Each decision must show `背景信息` and `决策摘要`;
  ...
  must say 谁继续处理, state the 不选推荐 cost, and show 真实差异 between options.
...
- Confirm every human-judgment node has tiered decision options, a clear `recommended_option`, required `decision_background` / `背景信息`, required `decision_summary` / `决策摘要`, required `benefit` / `收益`, required `cost` / `代价`, required actionable `next_exit`, required `consequence` as an execution field / 执行字段, and `recommendation_reason` / `推荐理由` on the recommended option. The option copy must explain what the reviewer chooses the model to do next, 谁继续处理 after selection, downstream impact on scope, schedule, risk, UI, plan, tasks, implementation, or tests, the cost of 不选推荐, and why the recommended option is safest. `must_confirm` nodes default to 3-4 options; a source-backed mutually exclusive binary decision may use 2 options only when `options_count_rationale` explains why no third executable exit exists; ordinary human-judgment nodes default to 3 options. Options in the same node must have 真实差异 and must be shaped by one of three decision templates: 范围决策, 门禁决策, or 降级决策. The wording must not repeat 模板句 / boilerplate across options; any visible 技术词 must have an immediate Chinese explanation / 中文说明. Confirm `.specify/review/scripts/validate-review-data.mjs` passes so lazy labels, repeated copy, missing owner, missing background/summary/tradeoff fields, missing why-now copy, generic business node summaries, and unlabeled decision exits are caught before closeout. Confirm every outgoing edge from a `decision` or `human_judgment` node states the business condition or result that selects the target node. Confirm saved options whose `next_exit` starts with `needs-decision` write to `needs_decision_items` (`OPTION_B.next_exit` must use this route and never counts as confirmed), saved concrete continuation routes such as `OPTION_A/C/D` write to `decision_recorded_items`, and unresolved nodes write to `unresolved_decision_items`. Do not use approve/defer/reject/block as the node-level primary interaction; statuses such as `BLOCKED` or `NEEDS_DECISION` may appear only as system results with a reason, owner, repair route, and exit path.
