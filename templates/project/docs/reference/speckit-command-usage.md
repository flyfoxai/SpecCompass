...
当 `/sp.flow` 或 `/sp.ui` 需要人工确认时，命令不能只写“请确认”。它必须先用简洁中文展示确认摘要：flow 要说明设计依据、业务目标、角色、主流程、决策点、异常/恢复、状态变化和需要看的标签；UI 要说明 PRD/spec/flow 依据、布局结构、screen/section、按钮和作用、字段和校验、图片/预览、图表/表格及数据源、权限/状态和需要看的标签。若其中有人工决策点，必须给分层可执行选项：`must_confirm` 3-4 项，普通人工判断默认 3 项，低风险二元判断只有说明为什么 2 项足够时才允许 2 项。每个选项都要说人话说明真实背景、选择后模型要做什么、会影响哪些后续范围/排期/风险/实现/测试，以及为什么推荐。
