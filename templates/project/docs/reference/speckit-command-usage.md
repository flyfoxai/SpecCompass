...
`/sp.prd` 使用递归 Outline 窗口，而不是固定三级项目职责。默认业务资料根是仓库根 `prd/`；人工可以追加或明确替换资料目录，命令必须记录实际来源根和锚点。`specs/<feature>/prd.md` 是当前 feature 的整理与决策落点，不是默认唯一资料。首次完整提取包含 `product_subject`、`business_objects`、`operations`、`outcomes`、`capability_atoms` 和 `business_chains` 的 `business_context` 并通过校验后，`specs/<feature>/prd/source-capability-baseline.json` 只保存来源指纹和 Stage A 事实；来源、当前人工修正和已接受决定未变化时必须原样复用，旧项目名称、数量和分区不会进入该文件。之后按五步完成项目分区：Stage B 把相互冲突且有文档或人工权威的 owner/lifecycle 当成不可跨越的合并约束；共享同一权威身份是保留在一起的重要证据，但不机械禁止内部拆分；baseline 中相同的 `ai-proposed` 身份只是软线索；模型先草拟真正不同的 Web 方案；通过权威门禁的方案再以“项目内部复杂度 + 项目间协调成本”比较；最后输出选中项目及证据。每套分区写明内部复杂度、协调成本和总复杂度名次，实际生成项必须排第 1；真正并列时允许多个第 1，选中项只是页面临时推荐，全部并列方案仍待人工决定。`000` 第一次只生成一层，普通单元通常生成两层或三层。单原子或没有第二套有效方案时不得制造假分区，不能因共享页面、数据库、运行时、阶段或团队而合并。多原子子单元只在全部原子真实共享时填写 owner/lifecycle，并保留一个覆盖全组的自然语言业务不变量和一个 separation test，完整列出最佳拆分备选、真实跨组事实、确有的重复状态以及保留归组总复杂度更低的原因。标题不能使用“责任单元/责任域”等 SP 内部分类，同级 frontier 不能复制同一套下一层拆分说明。模型提案保持 `ai-proposed` 与 `proposal_origin: current-discovery`，只引用原始业务资料、人工资料或确认父级依据，并进入 Web 保留/拆分审核。完整拥有接收、标准化、质量/版本/血缘、留存和可信交付的数据业务生命周期可以形成数据责任边界。不设固定数量，也不要求越细越好；三层仅是界面显示窗口，不是整棵树上限。

参考分区必须按身份来源标出 `hard`、`soft` 和整体 `provisional_authority`。全是 `ai-proposed` 时，reference 只是不随重跑漂移的临时默认，不是已确认项目边界；默认树仍用 reference，但展开根必须提供一级分区数量/归属问题，并在 Outline 标记 `Reference Authority: model-proposed, unconfirmed`。人工确认后，该分区才成为后续运行可继承的 `user-confirmed` 锚点。

schema v6 的每个 Outline 单元必须同时提供 `project_boundary` 项目边界合同：自有责任、范围、自有业务对象、非目标、上游输入、下游输出、独立验收和未决边界。每个单元还必须提供 `project_title`，由自有业务对象与 `overall_outcome` 派生为具体实体名；它不能是 owner/lifecycle slug、“X责任”“X生命周期”或“责任单元/责任域”等内部分类词。它必须复用至少一个自有对象的具体名词，命名该单元拥有并治理的记录或聚合（回答“它拥有什么”，而非“它做什么”）；不得是发布、授权、决定、裁决、披露、评测、审计、恢复、执行、采集、处理、管理、计算、路由、监控等动作或治理词（除非该词本身就是来源命名的自有对象），也不得用事实、证据、信息、数据、版本、特征、结果等伞词替换对象的具体名词。`unresolved_boundary` 必须描述本单元自己的边界张力，同级兄弟不得复用同一句模板。自有业务对象必须与本单元能力原子实际使用的对象完全一致，不能夹带无关对象。上下游合同分别覆盖本单元全部业务链，并引用支持对应业务链的来源。`grouping_basis` 只解释能力原子为何归在一起，不能替代项目定义；写不出具体项目合同的抽象分组不能进入候选项目树。模型生成的完整边界仍是 `ai-proposed`，必须在 Web Discovery 中确认后才能成为基线。
