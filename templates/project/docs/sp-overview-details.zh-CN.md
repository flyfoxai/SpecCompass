...
多 agent 是受控优化，不是默认执行模式。默认仍按单 agent 串行推进；只有任务边界清楚、写集不重叠、依赖已满足、检查明确时，才把任务标为 `[P]` 或交给 worker。
...
如果 worker 超时、输出为空、缺证据、修改越界、写集冲突、依赖 rejected/stale worker，或触碰未授权 shared/global 文件，应冻结批次。coordinator 需要分类 worker 状态、保留可验证证据、丢弃或延后不可验证结果，并转成单 agent 串行兜底。没有 fallback 报告的多 agent 批次不能支持 analyze/gate PASS。
