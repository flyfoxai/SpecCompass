...
- Keep acceptance anchors searchable from `memory/trace-index.md` and related tasks.
- Mark missing validation evidence as `@t0` only when the gap needs follow-up; record non-trivial gaps in `memory/open-items.md`.
- Tests and smoke checks should prove the current workset outcome, not just file existence.
- Do not claim readiness when a blocker or unowned high-impact risk still affects the acceptance path.
