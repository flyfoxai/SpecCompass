...
- Classify each blocker before deciding: `INFO_GAP`, `SOURCE_AUTHORITY_GAP`, `UPSTREAM_DOC_GAP`, `CODE_TEST_ONLY`, `EXECUTION_INFRA`, `GENERIC_ARTIFACT`, `BUSINESS_DECISION`, `ROUTING_STALE`, or `SCOPE_CONFLICT`.
- Do not pass when blocker classification, owner route, smallest solvable unit, verification path, or writeback target is missing.
- Do not pass when stale routing leaves the active feature, workset, or owner command ambiguous.
- Do not pass when source authority is missing and generated summaries, tests, or model confidence are being used as a substitute.
- Do not pass when generic template artifacts are being used as evidence for specific business flow, UI, delivery, task, or acceptance behavior.
- Do not pass when execution infrastructure failure prevents required evidence from being produced. Record it as `EXECUTION_INFRA` and return `BLOCKED` or `FAIL` as appropriate.
...
- Do not pass a document-stage gate when discovered code/test work is not represented as a next-stage `Mode: impl` handoff packet.
...
- When the same failure signature appears across many modules, group it as one root blocker family and require the root owner route before more broad/batch reruns.
