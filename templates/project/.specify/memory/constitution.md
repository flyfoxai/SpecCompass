...
- `sp.prd` must end with PRD-to-spec outline readiness. It should create or refresh `specs/<feature>/spec-outline.md` with `READY_FOR_SPECIFY` only when enough confirmed or source-backed intent exists. Otherwise route with `NEEDS_PRD`, `NEEDS_CLARIFY`, `NEEDS_SOURCE`, `SPLIT_REQUIRED`, or `BLOCKED`.
