...
- `sp.prd` is the mandatory upstream requirement intake for new feature work, capability directions, and important requirement changes. Simple requests may use a short PRD, but they must not skip PRD. It is not a stable fact source. `sp.specify` owns stable requirement intake, conflict checks, and baseline `spec.md` creation.
- `sp.prd` must end with PRD-to-spec outline readiness. When enough confirmed or source-backed intent exists, it must create or refresh `specs/<feature>/spec-outline.md` as `AWAITING_OUTLINE_CONFIRMATION`, generate `specs/<feature>/prd/review/outline-review-data.json`, and require graphical review. Only a git-trackable `specs/<feature>/prd/review/outline-confirmation.md` whose Review Data ID, `Outline Digest`, and `Source Authority IDs` match the current outline may promote it to `READY_FOR_SPECIFY`. Browser state, localStorage, preview completion, and download alone never authorize `sp.specify`. Otherwise route with `NEEDS_PRD`, `NEEDS_CLARIFY`, `NEEDS_SOURCE`, `SPLIT_REQUIRED`, `NEEDS_DECISION`, or `BLOCKED`.
...
- For broad, high-risk, or repeated blockers, diagnose the root layer before fallback and split the issue into the smallest solvable unit. A blocker handoff should carry `Blocker ID`, a stable failure signature, root layer, disconfirming evidence before a repeated attempt, verification path, writeback target, and next route.
- Current blocker, risk, decision, and close-condition state belongs in `memory/open-items.md`. `memory/trace-index.md` is relation/history lookup, and `fallback-log.md` only prevents repeated loops unless `/sp.analyze` or `/sp.gate` promotes it into open-items.
- `/sp.tasks`, `/sp.plan`, and `/sp.implement` may append fallback-log entries or `promote-candidate: <Failure Signature>` notes, but they must not directly create, merge, close, or promote `memory/open-items.md` blocker state. `/sp.analyze` and `/sp.gate` own fallback-log promotion and must cite an existing open item instead of creating duplicates when a signature was already promoted.
...
- When `NEEDS_DECISION` is reached, freeze downstream work for the same blocker until the human-selected decision is written back to the source doc, task, or `memory/open-items.md`. A model recommendation is not a decision record. The freeze record should name `Blocker ID`, `Failure Signature`, `Root Layer`, pending decision, frozen scope, `Writeback Target`, and next route.
...
- Closing, deleting, accepting, deferring, downgrading, or invalidating `Blocker`, high-impact `Risk`, or `@r0` requires `Close Evidence`: current verification, a traceable code/doc change, rollback/degrade evidence, or explicit human acceptance. If diff evidence is available, verify that the state change and its evidence changed together.
...
- Multi-agent execution is controlled execution, not the default. Use the canonical hard gates, worker handoff fields, worker status enum, dependency closure, fallback report fields, shared truth files, and global registry-like files defined in `docs/reference/sp-command-spec.md` §10.3.
- Parallel `[P]` tasks require explicit disjoint `Allowed Write Set` values, explicit `Required Checks`, satisfied dependencies, read-only shared truth files, serialized global registry-like changes, and coordinator baseline evidence before dispatch. If any gate is missing, broad, conflicting, or uncertain, run sequentially.
- The coordinator owns source docs, task DAG, shared truth, serial merge, final verification, fallback reports, and `sp.gate`. Workers own only assigned tasks or worksets and must return canonical handoffs with evidence and `Proposed Shared Updates` instead of editing shared truth directly.
- If worker output is stale, failed, unverifiable, out of bounds, conflicting, or dependent on a non-acceptable worker, freeze the batch: stop dispatching new worker work, stop merging unverified output, classify all workers, preserve evidence, and recover remaining work through single-agent sequential recovery.
- `sp.analyze` and `sp.gate` must not treat multi-agent work as PASS or CONDITIONAL until coordinator closeout verifies handoffs, actual diffs, forbidden writes, dependency closure, merged-state checks, serial shared-truth updates, and any required fallback report.
