...
description: Use when generating or repairing SpecCompass Flow, UI, or PRD Outline structured review data JSON for /sp.flow, /sp.ui, or /sp.prd confirmation pages.
...
Use this skill when `/sp.flow`, `/sp.ui`, or the Outline stage in `/sp.prd`
needs a confirmation page. The agent
...
普通 `/sp.flow`、`/sp.ui`、`/sp.prd` 只填数据 / fill only structured review data. They
...
- Outline review Web entry: `.specify/review/renderer/speccompass-review-renderer.html?outline=<feature>`
- Outline discovery Web entry: `.specify/review/renderer/speccompass-review-renderer.html?outline-discovery=<feature>`
...
`flow-review-data.json`, `ui-review-data.json`, or `outline-review-data.json`
from that feature's current
...
`ui-confirmation.md` for UI, and `prd/review/outline-confirmation.md` for
Outline. Outline remains owned by `/sp.prd`; this skill does not create a new
required `/sp.outline` command.

PRD Outline has an orthogonal `outline_maturity` contract:
...
Every discovery question uses 2-4 `candidates`, a recommendation and reason,
none of the above, and free-form input. It must expose the explicit operations
`confirm_candidate`, `add`, `replace`, `exclude`, and `context_note`. Do not
...
regenerates into temporary outputs, and fails closed unless every consumed
delta is in the expected section with `<!-- intent-delta:<id> -->`. New user
text uses `[src:user]`, accepted model candidates use `[src:user-confirmed]`,
and unaccepted candidates remain `[src:ai-proposed]`. Replacements, corrections,
and reversals are new events with `supersedes_delta_id` referencing an earlier
accepted event that is already consumed by the current formal PRD, not a
ledger-only pending event; missing or duplicate formal PRD anchors, forward references, and cycles
are rejected, and accepted events are never mutated in place. If temporary
validation fails on two consecutive attempts with no new evidence, stop
regeneration, retain the failed deltas and
diagnostics, and set the blocking Outline status to `NEEDS_DECISION`. Reset the
consecutive-failure count after accepting valid new evidence. The Constitution may
check or constrain confirmed Stage 3 facts, but must not invent target users,
product goals, business rules, or scope.

Existing PRD entries that may be replaced or excluded use
`<!-- intent-target:<id> -->`, and each replacement or exclusion block records
`<!-- intent-ref:<delta-id>:<target-or-candidate-id> -->`. After regenerating
both temporary files, run
...
the recorded owner is no longer running, recover the stale lock only after
acquiring `.outline-discovery-writeback.recovery.lock` and rechecking the main
lock identity. Both locks have unique ownership IDs; cleanup must preserve any
lock whose ownership has changed. If a dead process left both locks, fail
closed until an operator verifies no writeback is running and removes only the
...
missing, incomplete, stale, or identity-mismatched new-contract confirmation.
Compute that identity only after the complete JSON is written, using
`.specify/review/scripts/review-data-id.mjs`. Its canonical serialization
recursively sorts object keys, preserves array order, and covers every review
...
Outline `change_type` values: `REVISE_INTENT`, `REVISE_USERS`,
...
- Generate new Flow/UI/Outline review data with `schema_version: 2`. Keep
  ...
  node uses `critical`, `important`, or `normal`, while informational nodes
  omit the priority fields. A `critical` node must include a substantial
  `critical_basis` (severe impact and no safe reversible/default route) and a
  concrete `priority_reason` for the reviewer.
- Rank candidates before assigning `critical`. For `N` actionable nodes, the
  upper bound is `N == 0 ? 0 : min(3, max(1, ceil(N / 10)))`. This is not a
  quota: zero critical / 0 个 critical is valid and preferred when no point
  qualifies. Downgrade / 降级 every excess candidate to `important` before
  running validation; the validator must reject invalid data, never rewrite it.
- Critical decisions require individual / 逐项 confirmation. Exclude unresolved
  critical nodes from every bulk / 批量 recommended-option scope, including
  view, module, requirement, and confirmation-package preflight. Direct card
...
- Human-judgment option count is tiered by review type:
  ...
  - UI `must_confirm` nodes require 3-4 executable options. Only an ordinary,
    ...
    exit exists.
  - Outline `must_confirm` nodes use 2-4 executable options. Keep choices at
    product-intent, scope, scenario-coverage, source-authority, or readiness
    level; never offer Flow steps, UI screens/components, APIs, database models,
    implementation tasks, or solution design as Outline choices.
